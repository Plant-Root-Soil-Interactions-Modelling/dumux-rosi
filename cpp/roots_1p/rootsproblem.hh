 ...
 */
template <class TypeTag> class RootsProblem : public PorousMediumFlowProblem<TypeTag> {
  ...
  enum { bcDirichlet = 0, bcNeumann = 1 };
...
public:
  ...
  void userData(std::string name, const SolutionVector &sol) {
    ...
    for (const auto &e : elements(gridView)) {
      ...
      if (name == "pXylem") { // this is the segment xylem pressure as
                              // 0.5*(node1 + node2)
        auto i0 = vMapper.subIndex(e, 0, 1);
        auto i1 = vMapper.subIndex(e, 1, 1);
  ...
  /*
   * \brief Return the temperature within the domain in [K]. (actually needed?
   * why?)
   */
   ...
   * that the conserved quantity is created, negative ones mean that it
   * vanishes. E.g. for the mass balance that would be a mass rate in \f$ [ kg /
   * (m^3 \cdot s)] \f$.
   */
   ...
   */
  void addPointSources(std::vector<PointSource> &pointSources) const { pointSources = couplingManager_->lowDimPointSources(); }
   ...
   * the absolute rate mass generated or annihilate in kg/s. Positive values
   * mean that mass is created, negative ones mean that it vanishes.
   */
  template <class ElementVolumeVariables>
  void pointSource(PointSource &source, const Element &element, const FVElementGeometry &fvGeometry, const ElementVolumeVariables &elemVolVars,
                   const SubControlVolume &scv) const {
    source = 0;
    if (couplingManager_ != nullptr) {
      ...
      if (age > 0) {
        ...
        if (verbose > 1) {
          std::cout << "pointSource " << sourceValue << " " << source.quadratureWeight() << " " << source.integrationElement() << " " << krel << " "
                    << rootRadius << " " << kr << " " << pressure3D << " " << pressure1D << std::endl;
        }
      }
    }
  }
  ...
  //! sets the current simulation time [s] (within the simulation loop) for
  //! collar boundary look up
  ...
  //! if true, sets bc to Dirichlet at criticalCollarPressure (false per
  //! default)
  void setCritical(bool b) { critical_ = b; }
    ...
    for (const auto &e : elements(this->gridGeometry().gridView())) {
      ...
      for (const auto &scvf : scvfs(fvGeometry)) { // evaluate root collar sub control faces
        if (onUpperBoundary_(scvf.center())) {     // root collar
          ...
          actualTrans_ = std::min(potentialTrans_,
                                  criticalTranspiration); // actual transpiration rate [kg/s]
  ...
  /*!
   * Writes the actual transpiration into a text file. Call postTimeStep before
   * using it.
   *
   * 0 time [s], 1 actual transpiration [kg/s], 2 potential transpiration
   * [kg/s], 3 maximal transpiration [kg/s], 4 collar pressure [Pa], 5 - (0.) 6
   * simtime [s]
   */
  void writeTranspirationRate() {
    file_at_ << neumannTime_ << ", " << actualTrans_ << ", " << potentialTrans_ << ", " << maxTrans_ << ", " << collarP_ << ", " << 0. << ", " << time_ << "\n";
  }
   ...
   */
  void scvSoilMatricPotential_(const Element &element, const FVElementGeometry &fvGeometry, const ElementVolumeVariables &elemVolVars,
  ...
  void setCouplingManager(CouplingManager *cm) { couplingManager_ = cm; }

private:
  //! cm pressure head -> Pascal
  Scalar toPa_(Scalar ph) const { return pRef_ + ph / 100. * rho_ * g_; }

  bool onUpperBoundary_(const GlobalPosition &globalPos) const { // on root collar
    // std::cout << "onUpperBoundary_ " << globalPos[dimWorld - 1] << " " << this->gridGeometry().bBoxMax()[dimWorld - 1] << std::endl;
