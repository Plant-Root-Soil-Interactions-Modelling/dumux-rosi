...
public:
    ...
    virtual void initializeProblem() {
        ...
        for (const auto& e : Dune::elements(gridGeometry->gridView())) {
            ...
            auto gId =  globalIdSet.id(e);
            localCellIdx[global2contiguous[gIdx]] = eIdx;
        }
     ...
     */
    virtual void fillCellIdx() {        
        int n = checkInitialized();//check grid exists
      int rank = grid->leafGridView().comm().rank();
      int size = grid->leafGridView().comm().size();
    
      int nLocalEntity =  std::distance(grid->leafGridView().template begin<0, Dune::Interior_Partition>(), 
					grid->leafGridView().template end<0, Dune::Interior_Partition>());
      ...
      int nGlobalEntity_ =  grid->leafGridView().comm().template sum<int>(nLocalEntity);
    globalIdSet =  grid->globalIdSet();      /** retrieve globally unique Id set */ 
      
      std::vector<int> offset(size);
      std::fill(offset.begin(), offset.end(), 0);

      /** Share number of locally owned entities */
      grid->leafGridView().comm().template allgather<int>(&nLocalEntity, 1, offset.data());
      
     cellIdx.resize(nGlobalEntity_,0.);
     global2contiguous.resize(nGlobalEntity_);
	...
	std::vector<int>  sendbuf(offset[rank]);
      int lId = 0;
        for (const auto& e : elements(grid->leafGridView())) {
            auto gId =  globalIdSet.id(e);
			if (e.partitionType() == Dune::InteriorEntity)
          {
              sendbuf[globalcontrib] = gId;
              lId++; 
          }
        }
		grid->leafGridView().comm(). template allgatherv<int>(sendbuf.data(), 
                                            nLocalEntity,recvbuf.data(),cellIdx.data(), P.data());
        std::sort(cellIdx.begin(), cellIdx.end());
        for(int ii = 0; ii <cellIdx.size(); ii++)
        {
            global2contiguous[cellIdx[ii]] = ii;
        }
    }
...
//std::cout<<"1cyl, gridVariables->advanceTimeStep"<<std::endl;
    ...
    throw std::invalid_argument("SolverBase::getCells: Not implemented yet (sorry)");
        checkInitialized();
    ...
    throw std::invalid_argument("SolverBase::getNeumann: Not implemented yet (sorry)");
    ...
    throw std::invalid_argument("SolverBase::getLocal2globalPointIdx: Not implemented yet (sorry)");
