...
int main(int argc, char** argv) //try
{
	...
	int verbose = getParam<int>("Problem.verbose");
			...
			for (const auto& vertex : Dune::vertices(fvGridGeometry->gridView()) )
			{
				...
				cstot_css1 += CSS1 * rVal;
				cstot_css2 += x[dofIdxGlobal][7]*bulkSoilDensity * rVal;
				cstot_cls +=  C_S_W * 0.20621373 * rVal;
				cstot_tot += cstot;
				cstot_totRval += cstot * rVal;
				if(verbose)
				{
					std::cout<<"we get: "
					<<" r:"<<rVal<<" theta:"<< 0.20621373				
    ...
    if (mpiHelper.rank() == 0)
    {
		...
		Parameters::print();
		std::ofstream myfile_;
		std::string filestr = problem->name() + "_1p10cR_cyl_end."+extenction; // output file
