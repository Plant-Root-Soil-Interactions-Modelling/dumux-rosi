...
public:
	...
	: PorousMediumFlowProblem<TypeTag>(fvGridGeometry) {
		...
		source_.resize(3); // 3 equations (currently hard coded, where can I get the value?)
		source_[0] = nullptr;
		source_[1] = nullptr;
		source_[2] = nullptr;
...
//            }
		...
		initialSoilM_ = InputFileFunction("Soil.IC", "M", "MZ", 0., this->spatialParams().layerIFF()); // kg/m2
//		for (int i=0; i<100; i++) {
//		    std::cout << initialSoilC_.f(-i*0.02) << "\n";
//		}

		// Uptake params
		vMax_ =  getParam<Scalar>("RootSystem.Uptake.Vmax", 0.)/24./3600.*1e1; //  [g cm-2 day-1] -> [kg m-2 s-1]
		km_ = getParam<Scalar>("RootSystem.Uptake.Km", 0.)*1e3;  // [g cm-3] -> [kg m-3]
...
//                    f = std::min(f, omax);
	...
	NumEqVector source(const Element &element, const FVElementGeometry& fvGeometry, const ElementVolumeVariables& elemVolVars,
		...
		double m = 0.;
		if (source_[mucilIdx] != nullptr) {
			s = source_[mucilIdx]->at(eIdx)/scv.volume();
		}
		return NumEqVector({ h, s, m });
	}
