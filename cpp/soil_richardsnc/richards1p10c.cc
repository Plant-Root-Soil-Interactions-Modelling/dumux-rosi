...
int main(int argc, char** argv) //try
{
    ...
    using TypeTag = Properties::TTag::Richards10CCC; // Richards2CCC, Richards2CBox, (TypeTag is defined in the problem class richardsproblem.hh)
    ...
    if (tEnd > 0) { // dynamic problem
        ...
        try { // CheckPoints defined
            ...
            std::cout<< "richards1p10c.cc: no check times (TimeLoop.CheckTimes) defined in the input file\n";
        }
    } else { // static
    }
    ...
    if (mpiHelper.rank() == 0)
    {
		...
		std::string filestr = problem->name() + "_1p10cR_T0." + extenction; // output file
		...
		for (const auto& vertex : Dune::vertices(fvGridGeometry->gridView()))
		{
			...
			//std::cout <<x[dofIdxGlobal][0]<<", "<<(( x[dofIdxGlobal][0] - pRefPa)*100/rho__/g__) << ", ";
				for(int j = 1; j < 3; j++)
				{
					myfile_ << x[dofIdxGlobal][j]/18.<<", ";
					//std::cout << x[dofIdxGlobal][j]/18.<<", ";
				} 
	...
	double k_sorp = getParam<double>("Soil.k_sorp")*1e6;
	double CSSmax = getParam<double>("Soil.CSSmax")*1e6;
	double f_sorp = getParam<double>("Soil.f_sorp");
	int verbose = getParam<int>("Problem.verbose");
	
			double cstot_tot = 0;
			double cstot_totRval = 0;
			double cstot_cls = 0;
			double cstot_css1 = 0;
			double cstot_css2 = 0;
			std::cout<<"\n\n"<<timeLoop->time()
			...
			for (const auto& vertex : Dune::vertices(fvGridGeometry->gridView()) )
			{
				...
				double cstot = C_S_W * 0.20621373 				
								+ x[dofIdxGlobal][7]*bulkSoilDensity + CSS1;
				cstot_css1 += CSS1;
				cstot_css2 += x[dofIdxGlobal][7]*bulkSoilDensity ;
				cstot_cls +=  C_S_W * 0.20621373 ;
				cstot_tot += cstot;
				cstot_totRval += cstot ;
				if(verbose)
				{
					std::cout<<"we get: "
					<<" r:"<<rVal[0]<<" theta:"<< 0.20621373				
					<<"\n\t\tcls:"<< C_S_W * 0.20621373 				
					<<" css2:"<<x[dofIdxGlobal][7]*bulkSoilDensity // + problem->getSorp() * timeLoop->timeStepSize() 
					<<" css1:"<< CSS1
					<<" cs:"<< cstot<<" r_cs:"<<cstot
					<<std::endl;
				}
    ...
    if (tEnd>0)  { // dynamic
        ...
        do {
			...
			for (const auto& vertex : Dune::vertices(fvGridGeometry->gridView()))
			{
				const auto dofIdxGlobal = fvGridGeometry->vertexMapper().index(vertex);
				auto rVal = vertex.geometry().center();//[0];
				cstot_css1 += problem->getCSS1(dofIdxGlobal) ;
				cstot_css2 += x[dofIdxGlobal][7]*bulkSoilDensity ;
				cstot_cls += x[dofIdxGlobal][1]*problem->getTheta(dofIdxGlobal)* (1e6/18.) ;
				double cstot = x[dofIdxGlobal][1]*problem->getTheta(dofIdxGlobal)* (1e6/18.) 
								+ x[dofIdxGlobal][7]*bulkSoilDensity + problem->getCSS1(dofIdxGlobal) ;
				cstot_tot += cstot;
				cstot_totRval += cstot ;
				...
				{
					std::cout<<"we get: "
				<<" r:"<<rVal[0]<<' '<<rVal[1]<<' '<<rVal[2]
					<<" FCSS2:"<< problem->getSorp(dofIdxGlobal)<<" theta:"<<problem->getTheta(dofIdxGlobal)
					<<" RF:"<< problem->getRF(dofIdxGlobal)
					<<" FCSS2*dt:"<< problem->getSorp(dofIdxGlobal) *timeLoop->timeStepSize()   
					
					<<"\n\t\tcls:"<< x[dofIdxGlobal][1]*problem->getTheta(dofIdxGlobal)* (1e6/18.)				
					<<" css2:"<<x[dofIdxGlobal][7]*bulkSoilDensity // + problem->getSorp() * timeLoop->timeStepSize() 
					<<" css1:"<< problem->getCSS1(dofIdxGlobal)
					<<" cs:"<< cstot<<" r_cs:"<<cstot
					<<std::endl;
				}
			}
    ...
    if (mpiHelper.rank() == 0)
    {
		...
		Parameters::print();
		std::ofstream myfile_;
		std::string filestr = problem->name() + "_1p10cR_end."+extenction; // output file
