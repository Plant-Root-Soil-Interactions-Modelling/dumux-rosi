...
//                    f = std::min(f, omax); rho_
	...
	void bioChemicalReaction(NumEqVector &q, const VolumeVariables &volVars, double pos0 ) const
	{
		...
		//Scalar massOrMoleFraction = useMoles? volVars.moleFraction(h2OIdx, mucilIdx) : volVars.massFraction(h2OIdx, mucilIdx);
		//Scalar massOrMoleDensity = useMoles? volVars.molarDensity(h2OIdx) : volVars.density(h2OIdx);
		//[mol solution / m3 solution] * [mol solute / mol solution] = [mol solute / m3 solution]
		Scalar C_L = massOrMoleDensity(volVars, h2OIdx, true) * std::max(massOrMoleFraction(volVars,0, mucilIdx, true), 0.);//X/X I think to X/m3 soil water
		...
		Scalar C_Oa = bulkSoilDensity * std::max(massOrMoleFraction(volVars,0, CoAIdx - numFluidComps, false), 0.);//X/X I think to X/m3 bulk solid
