...
#include "./rootsystemEnableGravity/model.hh"
#include "./rootsystemEnableGravity/problem.hh"
...
public:
     ...
     void addOutputVtkFields()
    {
        ...
        ScalarField& xylemPH = *(this->resultWriter().allocateManagedBuffer(numDofs));
        source = 0.0;
        xylemPH = 0.0;
        Scalar totalSource;
        totalSource = 0.0;
        ...
        for (const auto& element : elements(this->gridView()))
        {
            ...
            for (int scvIdx = 0; scvIdx < fvGeometry.numScv; ++scvIdx)
            {
                ...
                if (!(this->timeManager().time() < 0))
                {
                    ...
                    totalSource += source[dofGlobalIdx];
                    xylemPH[dofGlobalIdx] += -(1e5-elemVolVars[scvIdx].pressure())/1000/9.81*100;
                }
            }
        }

        std::cout<<"Total uptake to root : "<<totalSource<<std::endl;
        // attach data to the vtk output
        this->resultWriter().attachDofData(source, "source (kg/s)", isBox);
        this->resultWriter().attachDofData(xylemPH, "xylemPH (cm)", isBox);
    }
