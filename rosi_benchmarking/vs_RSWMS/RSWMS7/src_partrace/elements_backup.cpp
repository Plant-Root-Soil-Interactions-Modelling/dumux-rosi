				 ...
				 vector3& pos, DispersionClass& dispersionData)
{
  ...
  // loop over boundary passings/reflections
	...
	static double LowerRec=0.0;
	static long LowerCount=0;
	static double UpperRec=0.0;
	static long UpperCount=0;
			...
			case 1: // left
				if(ex==0) {
					...
					reflectXYZ(dispersionData.ds);
						...
						if (fabs(1-dispersionData.dref_from[0]/dispersionData.dref_to[0])>1e-3 || fabs(1-dispersionData.dref_from[1]/dispersionData.dref_to[1])>1e-3 || fabs(1-dispersionData.dref_from[2]/dispersionData.dref_to[2])>1e-3) {
    ...
    case 5: // bottom
      if(ez==0) {
		...
		if (reflectDispersion(dispersionData.dref_from[2], dispersionData.dref_to[2]) ) {
			...
			if (fabs(1-dispersionData.dref_from[0]/dispersionData.dref_to[0])>1e-3 || fabs(1-dispersionData.dref_from[1]/dispersionData.dref_to[1])>1e-3 || fabs(1-dispersionData.dref_from[2]/dispersionData.dref_to[2])>1e-3) {
				LowerRec+=dispersionData.ds_to[2];
				LowerCount+=1;
				cout<<"Lower Rec "<< LowerRec/LowerCount << endl;
				UpperRec+=dispersionData.ds[2]/diff2*diff;
				UpperCount+=1;
				cout<<"Upper Rec "<< UpperRec/UpperCount << endl;
				cout<<"Rec coef "<< (LowerRec/LowerCount)/(UpperRec/UpperCount)<<endl;
			dispersionData.ds=dispersionData.ds_to;
			dispersionData.d_from=dispersionData.d_to;
			dispersionData.dref_from=dispersionData.dref_to; }
			if (dispersionData.ds[2]>0.0) {reflectXYZ(dispersionData.ds);} 
...

{
  ...
  double interpol[24];
	...
	double al;
	vector3 e;
	static vector3 memLower, memUpper;
	static int firstround=0;

	if (firstround==0) {
		memLower[0]=0.5; memLower[1]=0.5; memLower[2]=0.5;
		memUpper[0]=0.5; memUpper[1]=0.5; memUpper[2]=0.5;
		firstround+=1;
		}
...
//////////////////////// e_to //////////////////////////////////
  ...
  disp.vNorm_to=sqrt(disp.v_to[2]*disp.v_to[2]+disp.v_to[0]*disp.v_to[0]+disp.v_to[1]*disp.v_to[1]); 
	...
	if(disp.vNorm_to<vMin  || (dispersion_parameter_to[0]<vMin && dispersion_parameter_to[1]<vMin) ) {	
		...
		disp.ds_to[1]=disp.ds[1]*( disp.d_to[1]/disp.d_from[1]);
		disp.ds_to[2]=disp.ds[2]*( disp.d_to[2]/disp.d_from[2]);
		if ((ez==14 && e_from > e_to) || (ez==15 && e_to > e_from) ) {
			cout<<"disp.d_to[2]/disp.d_from[2] "<<disp.d_to[2]/disp.d_from[2]<<endl; 
			}
	}
/*
if (ez==14 && e_from > e_to) {
	memUpper[0]=disp.rand[0]; memUpper[1]=disp.rand[1]; memUpper[2]=disp.rand[2]; 
}
if (ez==15 && e_to > e_from) {
	memLower[0]=disp.rand[0]; memLower[1]=disp.rand[1]; memLower[2]=disp.rand[2]; 
}*/
	//if (ez>=13 && ez<=16) {
  ...
  disp.vNorm=sqrt(disp.v[2]*disp.v[2]+disp.v[0]*disp.v[0]+disp.v[1]*disp.v[1]); 
	...
	if(disp.vNorm<vMin  || (dispersion_parameter_from[0]<vMin && dispersion_parameter_from[1]<vMin) ) {	
		...
		switch(elements->ReflectionMethod) {
			...
			if ((ez==14 && e_from > e_to ) || (ez==15 && e_to > e_from)) {
			cout<<"disp.dref_to[2]/disp.dref_from[2] "<<disp.dref_to[2]/disp.dref_from[2]<<endl; 
			}
...
//////////////////////// SecDispersiveNew = randomize second dispersive displacement //////////////////////////////////
  ...
  if(disp.vNorm_to<vMin  || (dispersion_parameter_to[0]<vMin && dispersion_parameter_to[1]<vMin) ) {   
		...
		c1=sqrt( 2.0*Retardation*dt*disp.dmc_to );
    ...
    c1=disp.rand[1] * sqrt( 2.0*Retardation*dt*(d1) );
    // dispersion orthographical to v
    c2=sqrt( 2.0*Retardation*dt*(d2) );
    c3=disp.rand[2]*c2;
    c2*=disp.rand[0];
		
/*
if (ez_from==14 && e_to > e_from) {
    c1=memUpper[0] * sqrt( 2.0*Retardation*dt*(d1) );
    // dispersion orthographical to v
    c2=sqrt( 2.0*Retardation*dt*(d2) );
    c3=memUpper[2]*c2;
    c2*=memUpper[1];
}
if (ez_from==15 && e_from > e_to) {
    c1=memLower[0] * sqrt( 2.0*Retardation*dt*(d1) );
    // dispersion orthographical to v
    c2=sqrt( 2.0*Retardation*dt*(d2) );
    c3=memLower[2]*c2;
    c2*=memLower[1];
...
{ 
  ...
  double interpol[24];
	...
	double TriFactor[8];
	ElementsClass *elements=partrace->elements;
	static double dsSumLower=0.0;
	static double dsSumUpper=0.0;
	static long dsCountLower=0;
	static long dsCountUpper=0;
  ...
  if(disp.vNorm<vMin  || (disp.parameter[0]<vMin && disp.parameter[1]<vMin) ) {   
		// velocity==0, only diffusion
		c1=sqrt( 2.0*Retardation*dt*diffusion );
    ...
    // dispersion in v-direction
    c1=disp.rand[0] * sqrt( 2.0*Retardation*dt*(d1) );
    // dispersion orthographical to v
    c2=sqrt( 2.0*Retardation*dt*(d2) );
    c3=disp.rand[2]*c2;
    c2*=disp.rand[1];
    ...
    disp.ds[2]=( c1*disp.v[2]/disp.vNorm + c2*disp.v1[2] + c3*disp.v2[2] ) * DispWeight[2];
		...
		if (ez==14 && disp.ds[2]>0.0) {
			dsSumLower+=disp.ds[2];
			dsCountLower+=1;
			cout<<"dsMean Lower "<<dsSumLower/dsCountLower<<endl;
		}
		if (ez==15 && disp.ds[2]<0.0) {
			dsSumUpper+=disp.ds[2];
			dsCountUpper+=1;
			cout<<"dsMean Upper "<<dsSumUpper/dsCountUpper<<endl;
			cout << "Coef should be (lower/upper) "<< fabs( (dsSumLower/dsCountLower) / (dsSumUpper/dsCountUpper) ) <<endl;
		}
/*
					if (ex==15 && ey==0 && ez>12 && ez<17) {
