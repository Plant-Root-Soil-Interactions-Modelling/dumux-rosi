...
class SoilWrapper(pb.SoilLookUp):
    
    def __init__(self, dumux_soil):
        super(SoilWrapper, self).__init__()
        self.soil = dumux_soil
    
    def getValue(self, pos, organ):
        print("getValue")
        print(pos)
        ci = self.soil.pick([pos.x, pos.y, pos.z])
        print(ci)
        return self.soil.getSolutionAt(ci)
