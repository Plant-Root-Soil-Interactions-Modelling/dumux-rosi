...
sim_time = 1  # [day]
NT = 10 * 72  # 216  # 216  # iteration ( 3 days / 216 = 1200 s)
...
for i, s_ in enumerate(ana.segments):
    if i != s_.y - 1:
        print("Segment indices are mixed up")
        input()
...
print("Initial pressure head", s.getSolutionHeadAt(cci), s.getSolutionHeadAt(picker(0., 0., -15.)))

cell_volumes = s.getCellVolumes()
print("cell volume", cell_volumes[cci])
soil_water = np.multiply(np.array(s.getWaterContent()), cell_volumes)
cyl_water = 0.
print("SEGMENTS", len(r.rs.cell2seg[cci]))
...
# print("1213", nodes[segs[1213, 0], :], nodes[segs[1213, 1], :])
# print("seg index", segs[1213, 1] - 1)
# mid = 0.5 * (nodes[segs[1213, 0], :] + nodes[segs[1213, 1], :])
# index1213 = picker(mid[0], mid[1], mid[2])
# print(index1213)
