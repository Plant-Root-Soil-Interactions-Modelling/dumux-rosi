...
""" We solve Benchmark 1 (abc) with a 3D YaspGrid but no resolution in x and y (for speed), 
    also works parallel with mpiexec
"""


def toHead(pa):
    """ Pascal (kg/ (m s^2)) to cm pressure head """
...
def solve(number, s_):
    ...
    if rank == 0:
        if number == 'a':
            ax1.plot(toHead(x), points[:, 2] * 100, "r*")
        elif number == 'b':
            ax2.plot(toHead(x), points[:, 2] * 100, "r*")
        elif number == 'c':
            ax3.plot(toHead(x), points[:, 2] * 100, "r*")
