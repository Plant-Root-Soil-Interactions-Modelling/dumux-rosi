...
"""
Helper module to define age depent tabular values for root conductivities (values are hard coded)
...
after calling:
    init_conductivities(XylemFluxPython, age_dependent)
"""
...
def init_conductivities(r, age_dependent :bool = False):
    ...
    if age_dependent:
        r.setKrTables([kr0[:, 1], kr1[:, 1], kr1[:, 1], kr1[:, 1]], [kr0[:, 0], kr1[:, 0], kr1[:, 0], kr1[:, 0]])  # [cm^3/day]
        r.setKxTables([kx0[:, 1], kx1[:, 1], kx1[:, 1], kx1[:, 1]], [kx0[:, 0], kx1[:, 0], kx1[:, 0], kx1[:, 0]])  # [1/day]
        ...
        kx = np.array([[0, kx_const_], [14, kx_const_]])
        r.setKrTables([kr[:, 1], kr[:, 1], kr[:, 1], kr[:, 1]], [kr[:, 0], kr[:, 0], kr[:, 0], kr[:, 0]])  # [cm^3/day]
        r.setKxTables([kx[:, 1], kx[:, 1], kx[:, 1], kx[:, 1]], [kx[:, 0], kx[:, 0], kx[:, 0], kx[:, 0]])  # [1/day]
#         r.setKr([kr_const_])  # [cm^3/day] # todo check order of age (segs are sorted)
#         r.setKx([kx_const_])  # [1/day]

# # default ist age independent
