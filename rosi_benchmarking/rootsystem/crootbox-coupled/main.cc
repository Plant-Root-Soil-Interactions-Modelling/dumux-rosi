...
NEW_TYPE_TAG(SoilTypeTag, INHERITS_FROM(BoxModel, Richards));
...
void updateSaturation(std::vector<double>& saturation,
                      const SoilFVGridGeometry& gridGeometry,
                      const SoilGridVariables& gridVariables,
...
int main(int argc, char** argv) try
{
    ...
    using SoilGridCreator = typename GET_PROP_TYPE(SoilTypeTag, GridCreator);
    SoilGridCreator::makeGrid("Soil"); // pass parameter group (see input file)
    ...
    // set the tropism manually to hydrotropism
    const std::vector<double> sigma({0.4, 1.0, 1.0, 1.0, 1.0, 0.4, 1.0, 1.0, 1.0, 1.0});
    for (int i = 0; i < 10; ++i)
    {
        auto* p = rootSystem->getRootTypeParameter(i+1);
        p->dx = 0.5;
        p->tropismT = ::RootSystem::TropismTypes::tt_hydro;
        p->tropismN = 20;
        p->tropismS = sigma[i];
    }

    // soil grid lookup
    const auto& soilGridView = SoilGridCreator::grid().leafGridView();
    ...
    // set the soil lookup
    // TODO: Use actual saturation field
    ...
    auto box = std::make_unique<::SDF_PlantBox>(10, 10, 26); // cm (!)
    auto translatedBox = std::make_unique<::SDF_RotateTranslate>(box.get(), ::Vector3d(0, 0, -4)); // cm (!)
    rootSystem->setGeometry(translatedBox.get());
    ...
    auto rootProblem = std::make_shared<RootProblem>(rootGridGeometry, couplingManager);
    ...
    VtkOutputModule<SoilTypeTag> soilVtkWriter(*soilProblem, *soilGridGeoemtry, *soilGridVariables, sol[soilDomainIdx], soilProblem->name());
    GET_PROP_TYPE(SoilTypeTag, VtkOutputFields)::init(soilVtkWriter);
    soilVtkWriter.write(0.0);

    VtkOutputModule<RootTypeTag> rootVtkWriter(*rootProblem, *rootGridGeometry, *rootGridVariables, sol[rootDomainIdx], rootProblem->name());
    ...
    while (!timeLoop->finished())
    {
        ...
        // grow one time step
        growth.grow(timeLoop->timeStepSize());

        // after the growth we need to resize everything before we can compute again
        if (growth.hasGrown())
        {
            std::cout << "Resizing problem after growth." << std::endl;

            rootProblem->spatialParams().updateParameters(*rootSystem);
        ...
        // update old solution to new grid
        oldSol[rootDomainIdx] = sol[rootDomainIdx];
        rootVtkWriter.write(timeLoop->time()+1.0);
