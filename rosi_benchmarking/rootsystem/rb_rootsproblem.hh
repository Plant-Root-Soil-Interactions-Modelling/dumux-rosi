...
#include "rb_rootspatialparams.hh"
...
public:
	...
	RootsProblem(std::shared_ptr<const FVGridGeometry> fvGridGeometry) : ParentType(fvGridGeometry) {
		...
		soilP_ = toPa_(getParam<std::vector<Scalar>>("RootSystemParameter.SoilP"));
		soilTable_ = soilP_.size()>1;
		if (soilTable_) {
			soilZ_ = getParam<std::vector<Scalar>>("RootSystemParameter.SoilZ");
		}

		p0_ = toPa_(getParam<std::vector<Scalar>>("RootSystemParameter.P0"));
		collarTable_ = p0_.size()>2;
		collarSine_ = p0_.size() == 2;
		if (collarTable_) {
			pT_ = getParam<std::vector<Scalar>>("RootSystemParameter.PT");
		}
