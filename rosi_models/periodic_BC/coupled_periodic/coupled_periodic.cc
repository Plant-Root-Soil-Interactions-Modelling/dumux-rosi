...
#include <dumux/io/grid/rootsystemgridmanager.hh>
...
#include "properties_periodic.hh" // inclcudes root properties, soil properties, redefines coupling manager
...
int main(int argc, char** argv) try
{
    ...
    const auto soilLowerLeft = soilGridGeometry->bBoxMin();
    const auto soilUpperRight = soilGridGeometry->bBoxMax();
    ...
    if (simtype==Properties::dgf) { // for a static dgf grid
        ...
        PeriodicNetworkGridManager<3> rootGridManager(soilLowerLeft, soilUpperRight, periodic);
        rootGridManager.init("RootSystem"); 
        ...
        rootGridManager.init(*rootSystem, soilLowerLeft, soilUpperRight, periodic);
    ...
    const auto gridData = rootGridManager.getGridData();
    const auto periodicConnectivity = gridData->createPeriodicConnectivity(rootGridGeometry->elementMapper(), rootGridGeometry->vertexMapper());
