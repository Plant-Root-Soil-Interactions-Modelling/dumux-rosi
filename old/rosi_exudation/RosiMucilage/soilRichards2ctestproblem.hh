...
SET_TYPE_PROP(SoilRichardsTwoCTestProblem, Grid, Dune::YaspGrid<3, Dune::TensorProductCoordinates<typename GET_PROP_TYPE(TypeTag, Scalar), 3> >);
// explicitly set the fvgeometry as we currently use another one as in stable
...
//#endif
                                  ...
                                  const ElementVolumeVariables &elemVolVars) const
    {
        ...
        Scalar MaxValue = 3.86e-13;//kg/s
        Scalar Exdudation_value;
        if (rootAge >= 0)
            Exdudation_value = 2*M_PI*MaxValue*exp(-rootAge*5e-6); //random function
        else
            Exdudation_value = 0;
