           ...
           char **argv)
{
    ...
    radialParameterTree::tree().sub("BoundaryConditions")["OuterFluxesWater"] = argv[16];
    radialParameterTree::tree().sub("BoundaryConditions")["OuterFluxesNutrient"] = argv[18];
    radialParameterTree::tree().sub("BoundaryConditions")["RootPressure"] = argv[20];
    radialParameterTree::tree().sub("BoundaryConditions")["InitialSoilPressure"] = argv[52];
    radialParameterTree::tree().sub("radialSoilGrid")["Positions0"] = argv[14];
    std::cout<<"!!!! argv[14] "<< radialParameterTree::tree().template get<std::vector<Scalar>>("radialSoilGrid.Positions0")[0]<<" "
            <<radialParameterTree::tree().template get<std::vector<Scalar>>("radialSoilGrid.Positions0")[1]<<"\n";

    std::cout<<"!!!! GET_RUNTIME_PARAM_CSTRING "<< GET_RUNTIME_PARAM_CSTRING(TypeTag, ScalarVector, "radialSoilGrid.Positions0")[0]<<" "
                                                << GET_RUNTIME_PARAM_CSTRING(TypeTag, ScalarVector, "radialSoilGrid.Positions0")[1]<<"\n";
    ////////////////////////////////////////////////////////////
