...
public:
    ...
    void setFreundlichK(Scalar FreundlichK)
    { FreundlichK_ = FreundlichK; }

    void setFreundlichN(Scalar FreundlichN)
    { FreundlichN_ = FreundlichN; }

    void setBulkDensity(Scalar BulkDensity)
    { BulkDensity_ = BulkDensity; }
    ...
    void setRestartSolutions(std::vector<PrimaryVariables> restartSolutions)
    {   for (const auto& solution : restartSolutions)
        restartSolutions_.push_back({solution[0],solution[1]}); }
    ...
    void setFinalRhizoRadius(Scalar finalRhizoRadius)
    { finalRhizoRadius_ = finalRhizoRadius; }
    ...
    const auto createdArgv()
    {
        ...
        argv.push_back(outputName_);//4
        argv.push_back("-radialTimeManager.Restart");//5
        argv.push_back(to_string_with_precision(restartTime_));//6
        ...
        argv.push_back(to_string_with_precision(tEnd_));
        argv.push_back("-Rhizosphere.Grid.Positions0");
        argv.push_back(to_string_with_precision(rootRadius_)+" "+to_string_with_precision(finalRhizoRadius_*0.8)+" "+to_string_with_precision(outerBoundary_)); //14
        //std::cout<<"!!!! Coupling "<<to_string_with_precision(rootRadius_)+" "+to_string_with_precision(outerBoundary_)<<"\n";
        argv.push_back("-Rhizosphere.BoundaryConditions.OuterFluxesWater");
        argv.push_back(to_string_with_precision(fluxBoundaryCondition_[0])); //16
        argv.push_back("-Rhizosphere.BoundaryConditions.OuterFluxesNutrient");
        argv.push_back(to_string_with_precision(fluxBoundaryCondition_[1]));
        argv.push_back("-RootSystem.BoundaryConditions.InitialRootPressure");
        argv.push_back(to_string_with_precision(rootPriVars_[0])); //20
        argv.push_back("-RootSystem.InitialConditions.InitialSoluteMassFracInRoot");
        argv.push_back(to_string_with_precision(rootPriVars_[1]));
        argv.push_back("-RootSystem.BoundaryConditions.RootPressure");
        argv.push_back(to_string_with_precision(rootPriVars_[0]));
        argv.push_back("-Soil.VanGenuchten.VgAlpha");
        argv.push_back(to_string_with_precision(vgAlpha_));
        argv.push_back("-Soil.VanGenuchten.Vgn");
        argv.push_back(to_string_with_precision(vgn_));
        argv.push_back("-Soil.VanGenuchten.Swr");
        argv.push_back(to_string_with_precision(swr_));
        argv.push_back("-Soil.SpatialParams.Porosity");
        argv.push_back(to_string_with_precision(porosity_));
        argv.push_back("-Soil.SpatialParams.Permeability");
        argv.push_back(to_string_with_precision(permeability_));
        argv.push_back("-Soil.SpatialParams.BufferPower");
        argv.push_back(to_string_with_precision(buffer_));
        argv.push_back("-RootSystem.SpatialParams.Kr");
        argv.push_back(to_string_with_precision(kr_));
        argv.push_back("-RootSystem.SpatialParams.Vmax");
        argv.push_back(to_string_with_precision(Vmax_));
        argv.push_back("-RootSystem.SpatialParams.Km");
        argv.push_back(to_string_with_precision(Km_));
        argv.push_back("-RootSystem.SpatialParams.rootRadius");
        argv.push_back(to_string_with_precision(rootRadius_));
        argv.push_back("-RootSystem.SpatialParams.DiffCoeffRootMembrane");
        argv.push_back(to_string_with_precision(DiffCoeffRootMembrane_));
        argv.push_back("-RootSystem.SpatialParams.PartitionCoeff");
        argv.push_back(to_string_with_precision(partitionCoeff_));
        argv.push_back("-Soil.BoundaryConditions.InitialSoluteMassFracInSoil");
        argv.push_back(to_string_with_precision(initialSoilNutrient_));
        argv.push_back("-Soil.BoundaryConditions.InitialSoilPressure");
        argv.push_back(to_string_with_precision(initialSoilPressure_)); //52
        argv.push_back("-Rhizosphere.Grid.Z");
        argv.push_back(to_string_with_precision(Z_));
        argv.push_back("-Soil.SpatialParams.UseSoilData");
        argv.push_back("false");
        argv.push_back("-radialTimeManager.Restart");
        argv.push_back(restart_);
        argv.push_back("-Soil.SpatialParams.FreundlichK");
        argv.push_back(to_string_with_precision(FreundlichK_));
        argv.push_back("-Soil.SpatialParams.FreundlichN");
        argv.push_back(to_string_with_precision(FreundlichN_));
        argv.push_back("-Soil.SpatialParams.BulkDensity");
        argv.push_back(to_string_with_precision(BulkDensity_));
        argv.push_back("-RootSystem.SpatialParams.Cmin");
        argv.push_back(to_string_with_precision(Cmin_));
    ...
    const auto ActiveUptakeNumerical()
    {
        ...
        return Dumux::radialStart<ProblemTypeTag>(argv.size(), argvector, restartSolutions_);
    }

private:
    Scalar Km_, Vmax_, Cmin_, density_, rootRadius_, time_, Cinf_, Cinf_dl, effDiffCoeff_, saturation_, porosity_, buffer_, activeUptake_;
    Scalar restartTime_, tEnd_, dtInitial_, outerBoundary_, finalRhizoRadius_, Z_, vgAlpha_, vgn_, swr_,
             permeability_, kr_, partitionCoeff_, initialSoilPressure_, initialSoilNutrient_;
    std::string name_,outputName_, restart_;
    PrimaryVariables fluxBoundaryCondition_, rootPriVars_;
    std::vector<std::vector<Scalar>> restartSolutions_;
    Scalar DiffCoeffRootMembrane_, FreundlichN_, FreundlichK_, BulkDensity_;
