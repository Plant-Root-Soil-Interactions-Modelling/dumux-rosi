...
#include <dune/grid/io/file/dgfparser.hh>  // dune grids
...
#include "richardsparams.hh" // should know if we want to use head
...
template <class TypeTag> class MyRichardsProblem;
...
NEW_TYPE_TAG(MyRichardsProblem, INHERITS_FROM(Richards, RichardsParams));
NEW_TYPE_TAG(RichardsBoxProblem, INHERITS_FROM(BoxModel, MyRichardsProblem));   // box is the discretisation method
NEW_TYPE_TAG(RichardsCCProblem, INHERITS_FROM(CCModel, MyRichardsProblem));     // cc = cell centered is the discretisation method

SET_TYPE_PROP(MyRichardsProblem, Grid, Dune::FoamGrid<1,1>); // start with 1D FoamGrid<1,1>
...
SET_TYPE_PROP(MyRichardsProblem, Problem, Dumux::MyRichardsProblem<TypeTag>);

// Set the wetting phase
SET_PROP(MyRichardsProblem, WettingPhase)
{
private:
...
// Enable gravity
SET_BOOL_PROP(MyRichardsProblem, ProblemEnableGravity, true);

SET_BOOL_PROP(MyRichardsProblem, UseHead, false); // head is not working, i dont know why
...
class MyRichardsProblem : public RichardsProblem<TypeTag>
{
    ...
    typedef typename GET_PROP_TYPE(TypeTag, TimeManager) TimeManager; // in propertydefaults.hh (no idea, where this is included)
...
public:
     ...
     */
    MyRichardsProblem(TimeManager &timeManager, const GridView &gridView) : ParentType(timeManager, gridView)
{
        ...
        if (bcTop_==4) {
            precTime_ = GET_RUNTIME_PARAM_FROM_GROUP(TypeTag, vector<double>, Climate, Times);
            precData_ = GET_RUNTIME_PARAM_FROM_GROUP(TypeTag, vector<double>, Climate, Precipitation); // in [cm / s]
        }
        ...
        try { // Episodes defined
            episodeTimes_ = GET_RUNTIME_PARAM_FROM_GROUP(TypeTag, vector<double>, TimeManager, Episodes);
            ...
            try { // Number of episodes defined
                ...
                episodeTimes_  = vector<double>(); // create empty vector
                auto it = episodeTimes_.begin();
                episodeTimes_.insert(it,0.); // add initial
                double tend = GET_RUNTIME_PARAM_FROM_GROUP(TypeTag, double, TimeManager, TEnd);
                episodeTimes_.push_back(tend); // add final
    ...
    void episodeEnd()
    {
        if (!this->timeManager().willBeFinished()) {
            double dt = episodeTimes_.at(episodeNumber_)-episodeTimes_.at(episodeNumber_-1);
            //            cout << "\n\nNext Episode: time=" << this->timeManager().time() << " s , #"<< episodeNumber_ <<", dt=" << dt
            //                    << " from " << episodeTimes_.at(episodeNumber_-1) << " to " << episodeTimes_.at(episodeNumber_) <<"\n";
            episodeNumber_++;
            this->timeManager().startNextEpisode(dt);
        }
    }
...
private:
    ...
    double bcTopValue_;
    double bcBotValue_;
    vector<double> precTime_;
    vector<double> precData_;

    double pnRef_; // reference pressure
    string name_; // problem name
    vector<double> episodeTimes_;
    int episodeNumber_;
