...
def run_soybean(file_name, enviro_type, sim_time, kr, kx, lmax1, lmax2, lmax3, theta1, r1, r2, a, src):
    ...
    print("soybean scenario set", flush=True)
    ...
    print("net infiltration set", flush=True)
    ...
    print("soil model set", flush=True)
    ...
    print("hydraulic model set", flush=True)
    
    scenario.init_lupine_conductivities(r, kr, kx)
    print("conductivities set", flush=True)

    """ sanity checks """
    r.test()  # sanity checks
    print("sanity test done", flush=True)
