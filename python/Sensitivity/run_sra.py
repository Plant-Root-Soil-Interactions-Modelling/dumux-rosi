...
def run_soybean(exp_name, enviro_type, sim_time, mods, result_path, save_all=True):
    ...
    if "domain_size" in mods:
        ...
        min_b = -np.array([domain_size[0] / 2.0, domain_size[1] / 2.0, domain_size[2]])
        max_b = np.array([domain_size[0] / 2.0, domain_size[1] / 2.0, 0.0])
    ...
    if mods["conductivity_mode"] == "from_mecha":
        anatomy_ind = scenario.sort_indices(mods)
        anatomy = [scenario.get_anatomy(i) for i in anatomy_ind]
        radii = [mods["a145_a"], mods["a2_a"], mods["a3_a"]]
        assert np.isclose(float(cdata[0, 2, 2]), radii[0], atol=10 ** (-5)), "radii failed for typ 145"  # to test (can be removed later...)
        assert np.isclose(float(cdata[1, 2, 2]), radii[1], atol=10 ** (-5)), "radii failed for typ 2"
        assert np.isclose(float(cdata[2, 2, 2]), radii[2], atol=10 ** (-5)), "radii failed for typ 3"
    ...
    try:
        r1, r2, r3 = sra_new.simulate_dynamic(s, r, table_name, sim_time, dt, trans_, output_times, initial_age=initial_age, cc_data=cc_data)
    except:
        print("***********************")
...
def run(argv):
    ...
    if type == "original":  # relevant parameters
        ...
        mods = {
            ...
            "a2_a": a2,
            "a3_a": a3,
            "hairsZone145_a": hairsZone145,
            "hairsZone2_a": hairsZone145,
            "hairsZone3_a": hairsZone145,
            "hairsLength145_a": hairsLength145,
            "hairsLength2_a": hairsLength2,
            "hairsLength3_a": hairsLength3,
        }
        if "_free" in exp_name:
            mods["bot_bc"] = "freeDrainage"  # otherwise potential
        if "_200" in exp_name:
