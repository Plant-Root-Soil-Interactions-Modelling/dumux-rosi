...
def maize(i: int):
    ...
    min_b = [-38.0, -8.0, -200.0]  # data from INARI
    max_b = [38.0, 8.0, 0.0]
    cell_number = [1, 1, 200]
    area = 76.0 * 16  # cm2
...
def soybean(i: int):
    ...
    """
    soil, table_name = vg_enviro_type(i)
    min_b = [-38, -1.5, -200.0]  # data from INARI
    max_b = [38, 1.5, 0.0]
    cell_number = [1, 1, 200]
    area = 76 * 3  # cm2
...
def init_conductivities_const(r, kr_const=1.8e-4, kx_const=0.1):
    ...
    r.set_kr_const(0.0, 0)  # artificial shoot
    r.set_kx_const(1.0e3, 0)
    r.set_kr_const(kr_const, [1, 2, 3, 4, 5])  # subTypes = 1-5
    r.set_kx_const(kx_const, [1, 2, 3, 4, 5])


def init_lupine_conductivities3(r, ykr1, okr1, ykr2, okr2, ykr3, okr3, ykx1, okx1, ykx2, okx2, ykx3, okx3, yt1, yt2, yt3, delta1=1, delta2=2, delta3=3):
    ...
    r.set_kr_age_dependent([0.0], [0.0], 0)  # artificial shoot
    r.set_kx_age_dependent([0.0], [1.0e3], 0)
    kx1 = np.array([[0.0, ykx1], [yt1, ykx1], [yt1 + delta1, okx1], [yt1 + delta1, okx1]])  # increasing with age
    kr1 = np.array([[0.0, ykr1], [yt1, ykr1], [yt1 + delta1, okr1], [yt1 + delta1, okr1]])  # decreasing with age
    kx2 = np.array([[0.0, ykx2], [yt2, ykx2], [yt2 + delta2, okx2], [yt2 + delta2, okx2]])  # increasing with age
    kr2 = np.array([[0.0, ykr2], [yt2, ykr2], [yt2 + delta2, okr2], [yt2 + delta2, okr2]])  # decreasing with age
    kx3 = np.array([[0.0, ykx3], [yt3, ykx3], [yt3 + delta3, okx3], [yt3 + delta3, okx3]])  # increasing with age
    kr3 = np.array([[0.0, ykr3], [yt3, ykr3], [yt3 + delta3, okr3], [yt3 + delta3, okr3]])  # decreasing with age
...
def init_lupine_conductivities_sa(r, ykr1, okr1, ykr2, okr2, kr3_, ykx1, okx1, ykx2, okx2, kx3_):
    ...
    r.set_kr_age_dependent([0.0], [0.0], 0)  # artificial shoot
    r.set_kx_age_dependent([0.0], [1.0e3], 0)
    kx1 = np.array([[0.0, ykx1], [14.0, ykx1], [28.0, okx1], [28.0, okx1]])  # increasing with age
    kr1 = np.array([[0.0, ykr1], [7.0, ykr1], [28.0, okr1], [28.0, okr1]])  # decreasing with age
    kx2 = np.array([[0.0, ykx2], [7.0, ykx2], [14.0, okx2], [14.0, okx2]])  # increasing with age
    kr2 = np.array([[0.0, ykr2], [7.0, ykr2], [14.0, okr2], [14.0, okr2]])  # decreasing with age
    kr3 = np.array([[0.0, kr3_], [100.0, kr3_]])  # constant
    kx3 = np.array([[0.0, kx3_], [100.0, kx3_]])  # constant
    r.set_kr_age_dependent(kr1[:, 0], kr1[:, 1], [1, 4, 5]),  # age, value, subType
    r.set_kx_age_dependent(kx1[:, 0], kx1[:, 1], [1, 4, 5])
    r.set_kr_age_dependent(kr2[:, 0], kr2[:, 1], 2)
    r.set_kx_age_dependent(kx2[:, 0], kx2[:, 1], 2)
...
def init_lupine_conductivities(r, skr=1.0, skx=1.0):
    ...
    kr01 = np.minimum(skr * kr0[:, 1], 1.0)
    kr11 = np.minimum(skr * kr1[:, 1], 1.0)
    kx01 = np.minimum(skx * kx0[:, 1], 1.0)
    kx11 = np.minimum(skx * kx1[:, 1], 1.0)
...
def set_conductivities(params, mods, data):
    ...
    if cm == "age_dependent":
        ...
        I = np.argsort(-a)  # descending regarding radius a
        init_lupine_conductivities3(params, ykr[0], kr[0], ykr[1], kr[1], ykr[2], kr[2], ykx[0], kx[0], ykx[1], kx[1], ykx[2], kx[2], yt1, yt2, yt3)
        mods.pop("mecha_path")
...
def get_anatomy(index):
    ...
    aerenchyma_percentage = [0.0, 0.07, 0.15, 0.22, 0.30]
    ...
    index = int(index)
    n = (
        len(aerenchyma_number_of_areas)
        * len(cortex_layers)
        * len(cortex_cell_diameter)
        * len(stele_diameter)
        * len(stele_cell_diameter)
        * len(xylem_number_of_poles)
        * len(xylem_vessel_diameter)
    )
    ...
    r = np.array(
        [
            aerenchyma_percentage[aep_ind],
            aerenchyma_number_of_areas[ana_ind],
            cortex_layers[cl_ind],
            cortex_cell_diameter[ccl_ind],
            stele_diameter[sd_ind],
            stele_cell_diameter[scd_ind],
            xylem_number_of_poles[xnp_ind],
            xylem_vessel_diameter[xvd_ind],
        ]
    )
