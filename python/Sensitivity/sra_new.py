...
def simulate_dynamic(s, r, lookuptable_name, sim_time, dt, trans_f, output_times, initial_age=1.0, cc_data=None):
    ...
    for i in range(0, N):
        ...
        hsb_ = np.maximum(hsb_, np.ones(hsb_.shape) * -15000.0)  ############################################ (too keep within table)
        ...
        inner_kr_ = np.maximum(inner_kr_, np.ones(inner_kr_.shape) * 1.0e-7)  ############################################ (too keep within table)
        inner_kr_ = np.minimum(inner_kr_, np.ones(inner_kr_.shape) * 1.0e-4)  ############################################ (too keep within table)
        ...
        while err_ > 1 and c < matimes_iter:
            ...
            rx = np.maximum(rx, np.ones(rx.shape) * -15000.0)
        ...
        if i % skip == 0:
            ...
            print(
                "age {:g}".format(initial_age + t),
                "{:g}/{:g} {:g} iterations, rmse {:g}".format(i, N, c, err_),
                "; wall times {:g} {:g}".format(wall_interpolation / (wall_interpolation + wall_xylem), wall_xylem / (wall_interpolation + wall_xylem)),
                "number of segments",
                rs.getNumberOfSegments(),
                "collar potential {:g}".format(rx[0]),
                flush=True,
            )
