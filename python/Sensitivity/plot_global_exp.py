        ...
        for j in range(0, n_neurons):
            ...
            for k in range(0, N):
                ...
                if pareto:
                    ...
                    run_cplantbox_exp.show_me_file(
                        all[ind]["exp_name"], "results_cplantbox/", vis_hairs=True, mode="png", png_name="test({:g}, {:g})_{:g}".format(j, i, k)
                    )
                    c += 1
...
def exemplify_closest(all, node2sample, m_neurons, n_neurons, keys, vis_hairs=True):
    ...
    for i in range(0, m_neurons):
        for j in range(0, n_neurons):
            ...
            run_cplantbox_exp.show_me_file(
                all[ind[min_i]]["exp_name"], "results_cplantbox/", vis_hairs=True, mode="png", png_name="test({:g}, {:g})_mid".format(j, i)
            )
