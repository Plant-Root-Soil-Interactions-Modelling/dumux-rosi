...
from matplotlib.patches import Ellipse, Polygon, RegularPolygon
...
def rstd_(data, feature_names):
    ...
    rstd_ = [rstd_.append(100.0 * np.std(data[:, i]) / np.mean(data[:, i])) for i in range(0, data.shape[1])]
...
def print_info(data, feature_names):
    ...
    for i, name in enumerate(feature_names):
        ...
        print(
            "{:s}\t{:10.4f}\t{:10.4f}\t{:10.4f}\t{:10.4f}\t{:10.4f}".format(
                name.ljust(20),
                np.min(data[:, i]),
                np.max(data[:, i]),
                np.mean(data[:, i]),
                np.std(data[:, i]),
                100.0 * np.std(data[:, i]) / np.mean(data[:, i]),
            )
        )
        rstd_.append(100.0 * np.std(data[:, i]) / np.mean(data[:, i]))
...
def plot_hexagons(img, ax, f=cm.viridis):
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            wy = yy[i] * np.sqrt(3) / 2
            hex = RegularPolygon((xx[j] + 0.5 * (i % 2), wy), numVertices=6, radius=0.95 / np.sqrt(3), facecolor=f(img[i, j]), alpha=1.0, edgecolor="gray")
            ax.add_patch(hex)
...
def plot_rose(data, i, j, m_neurons, n_neurons, ax, f=cm.jet):
    ...
    for k, d in enumerate(data):
        ...
        tri = Polygon([[x0, y0], [x1, y1], [x2, y2], [x0, y0]], facecolor=f(k / len(data)), alpha=1.0, edgecolor="gray")

        ax.add_patch(tri)
        ax.text(x0, y0 + 0.37, "({:g},{:g})={:g}".format(j, i, i * n_neurons + j), size=24)  # why j, i? (som did it)
...
def plot_targets(all, target_names, m_neurons, n_neurons, node2sample, sample2node, mode="quad"):
    ...
    for k, _ in enumerate(target_names):
        ...
        if mode == "quad":
            ...
            img_.append(img)
            plot_hexagons(img, ax_[0], f=lambda c: [1.0, 1.0, 1.0])
