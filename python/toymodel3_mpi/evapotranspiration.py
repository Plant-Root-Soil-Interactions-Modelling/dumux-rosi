...
def net_infiltration(year, soil_type, genotype, sim_time, Kc):
    ...
    LAI =  df3[st+"_"+genotype].values #.loc[0:sim_time]
    day4LAI =  df3["day"].values
    f = interpolate.interp1d(day4LAI, LAI)
...
def decay(soil_sol_fluxes, dt, decay = 0.):
    ...
    for k in soil_sol_fluxes.keys():
        ...
        # print(k, decay, dt, soil_sol_fluxes[k],  (1.-decay*dt)*soil_sol_fluxes[k])
        if isinstance(soil_sol_fluxes[k], float):
            soil_sol_fluxes[k] = max(0., (1.-decay*dt)*soil_sol_fluxes[k])
        else:
