...
while rs_age < simMax: #for i, dt in enumerate(np.diff(times)):
    ...
    rs_age += dt
    print("Day",rank, rs_age)
    seg2cell_old = rs.seg2cell
    Ntbu = Nt
    
    if len(rs.cyls) > 1:
        cell_volumes =  comm.bcast(s.getCellVolumes() , root = 0) #cm3
        print('check old soil before plant growth', rank)
        rs.checkMassOMoleBalance2( sourceWat = np.full(len(cell_volumes),0.), # cm3/day 
                                     sourceSol = np.full((rs.numComp, len(cell_volumes)),0.), # mol/day
                                     dt = 0.,        # day    
    ...
    if False:# won t work: need to broadcast data before ehckking.
        if len(rs.cyls) > 1:
            cell_volumes =  comm.bcast(s.getCellVolumes() , root = 0) #cm3
            print('check old soil before parse plant data', rank)
            rs.checkMassOMoleBalance2( sourceWat = np.full(len(cell_volumes),0.), # cm3/day 
                                         sourceSol = np.full((rs.numComp, len(cell_volumes)),0.), # mol/day
                                         dt = 0.,        # day    
                                         seg_fluxes = 0.,# [cm3/day]
    ...
    if rank == 0:    
        if len(rs.cyls) > 1:
            ...
            write_file_float("segIdxCyl",rs.eidx[1], directory_ =results_dir)
            write_file_array("pressureHeadcyl",np.array(cyl.getSolutionHead()).flatten(), directory_ =results_dir)
            write_file_array("coordcyl", cyl.getDofCoordinates().flatten(), directory_ =results_dir)
            for i in range(rs.numFluidComp):
                write_file_array("solute_conc_cyl_"+str(i+1), np.array(cyl.getSolution(i+1)).flatten()* rs.molarDensityWat_m3/1e6 , directory_ =results_dir) 
            for i in range(rs.numFluidComp, rs.numComp):
                write_file_array("solute_conc_cyl_"+str(i+1), np.array(cyl.getSolution(i+1)).flatten()* rs.bulkDensity_m3 /1e6 , directory_ =results_dir) 
    ...
    print('out of inner loop',rank)

    cell_volumes =  comm.bcast(s.getCellVolumes() , root = 0) #cm3
    print('check old soil after leaving inner loop', rank)
    rs.checkMassOMoleBalance2( sourceWat = np.full(len(cell_volumes),0.), # cm3/day 
                                 sourceSol = np.full((rs.numComp, len(cell_volumes)),0.), # mol/day
                                 dt = 0.,        # day    
                                 seg_fluxes = 0.,# [cm3/day]
    ...
    thetaAfter = s.getWaterContent()
       
    
    if rank == 0:
        buWAfter = sum(np.multiply(np.array(thetaAfter), cell_volumes)) 
    ...
    if True:        
        ...
        comm.barrier()    
        print('write files for rank_0',rank)
        if rank == 0:
        ...
        comm.barrier()
    comm.barrier()
    print('check old soil before next time step', rank)
    rs.checkMassOMoleBalance2( sourceWat = np.full(len(cell_volumes),0.), # cm3/day 
                                 sourceSol = np.full((rs.numComp, len(cell_volumes)),0.), # mol/day
                                 dt = 0.,        # day    
                                 seg_fluxes = 0.,# [cm3/day]
