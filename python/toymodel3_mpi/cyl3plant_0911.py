...
def simulate_const(s, rs, sim_time, dt, kexu, rs_age,repartition, type,Q_plant,
    ...
    rs_age
    """
    Q_Exud = Q_plant[0]; Q_mucil = Q_plant[1] 
    ...
    for i in range(0, N):
        ...
        assert (abs(((new_soil_water - soil_water )[cellIds] - (np.array(list(soil_fluxes.values()))*dt + net_flux[cellIds]*dt))/((new_soil_water - soil_water )[cellIds])) < 0.1).all()
        ...
        if "dirichlet" in type:        
            r.solve(dt, seg_rx, proposed_outer_fluxes, seg_sol_fluxes, proposed_outer_sol_fluxes, seg_mucil_fluxes) #
        else:
            # solute fluxes are in mol/cm2 scv/d
            r.solve(dt, seg_fluxes, proposed_outer_fluxes, seg_sol_fluxes,proposed_outer_sol_fluxes, seg_mucil_fluxes) # 
