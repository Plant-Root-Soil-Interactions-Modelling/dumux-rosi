...
# testing the mass balance for the 1D axyssimetric soil
r_in = 2  # cm
r_out = 10
length = 3
...
def solve(simtimes, N):
    ...
    for r, dt in enumerate(dt_):
        ...
        rootSoilFluxes = s.getInnerFluxes(0, length) * dt # cm3
        rootSoilFluxesS = s.getInnerFluxes(1, length) * dt # kg
        soilSoilFluxes = s.getOuterFluxes(0, length) * dt # cm3
        soilSoilFluxesS = s.getOuterFluxes(1, length) * dt # kg
        
        # currently, setSource not properly implemented for richards and richards 2c
        ...
        todo:
        - check for 1D
        - update python wrapper so that getInnerBC and so on give the good stuff. 
        """
        print(s.base.innerIdx)
        raise Exception
