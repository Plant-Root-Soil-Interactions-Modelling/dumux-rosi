...
cell_number = [20, 20, 38]  # [8, 8, 15]  # [8, 8, 15]  # [16, 16, 30]  # [32, 32, 60]  # [8, 8, 15]
...
dt = 15 / (24 * 3600)  # [days] Time step must be very small
skip2 = 240 # Tobi: for skipping written vtu & vtps, skip=10 means every 10th time step is saved as result
...
for i in range(0, N):
    ...
    if i % skip2 == 0:
    	...
    	os.rename(name + "-00000.vtu", name +"_"+str(round(i/skip2))+".vtu")
    	ana = pb.SegmentAnalyser(r.rs)
    	ana.addData("xylem pressure", rx)
    	ana.write(name +"_"+str(round(i/skip2))+".vtp", ["radius", "xylem pressure", "creationTime", "subType"])
    	#ana.write("test.dgf", ["radius", "subType", "order"])
