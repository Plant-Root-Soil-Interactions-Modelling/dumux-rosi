...
min_b = [-6.25, -1.5, -180.]
max_b = [6.25, 1.5, 0.]
cell_number = [13, 3, 180]  #  [8, 8, 15]  # [16, 16, 30]  # [32, 32, 60]  # [8, 8, 15]
periodic = True
fname = "../../grids/spring_barley_CF12_30d.rsml"		# spring_barley_CF12_30d, RootSystem8

name = "barley_periodic_aged"
sand = [0.045, 0.43, 0.15, 3, 1000]
loam = [0.08, 0.43, 0.04, 1.6, 50]
clay = [0.1, 0.4, 0.01, 1.1, 10]
soil = loam

initial = -5000 + 7.5  # -659.8
...
if rank == 0:
    ...
    ax1.plot(x_, trans * sinusoidal(x_), 'k', label="potential")  # potential transpiration
    ax1.plot(x_, -np.array(y_), 'g', label="soil uptake")  # actual transpiration  according to soil model
    ax1.plot(x_, -np.array(cf), 'r:', label="root uptake")  # actual transpiration  according to root model
    ax1.set_xlabel("Time [d]")
    ax1.set_ylabel("Transpiration $[cm^3 d^{-1}]$")
    ax2 = ax1.twinx()
    dt = np.diff(x_)
    so = np.array(cf)
    ax2.plot(x_[1:], np.cumsum(-np.multiply(so[:-1], dt)), 'c--')  # cumulative transpiration (neumann)
    ...
    np.savetxt("results/" + name, np.vstack((x_, -np.array(cf))), delimiter=';')
