...
def launchUQR(directoryN,simInit, condition):
    ...
    def weather(simDuration, hp):
        ...
        weatherVar = {'TairC' : TairC_,'Pair':Pair,
    ...
    def resistance2conductance(resistance,r):
        ...
        r_bl = r_bl * r.R_ph * weatherX["TairK"] / r.Patm # [s/cm] * [K] * [hPa cm3 K−1 mmol−1] * [hPa] = [s] * [cm2 mmol−1]
        r_bl = r_bl * (1000) * (1/10000)# [s cm2 mmol−1] * [mmol/mol] * [m2/cm2] = [s m2 mol−1]
        return 1/r_bl
    ...
    while simDuration < simMax: 
        ...
        ##resistances
        r.g_bl = resistance2conductance(weatherX["rbl"])
        r.g_canopy = resistance2conductance(weatherX["rcanopy"])
        r.g_air = resistance2conductance(weatherX["rair"])
