...
def solve(soil, simtimes, q_r, N):
    ...
    # plt.show()    
    plt.savefig(soil[-1]+"_{:g}.png".format(qr))
...
if __name__ == "__main__":
    ...
    sim_times = np.linspace(0, 7, 24*6*7)  # temporal resolution of 0.1 h
    QRN = 4
    
    # qr_ = 100*np.linspace(0.01, 0.1, QRN)  / sim_times[-1]  # [cm/day]
    # soil = sand
    ...
    qr_ = 100*np.linspace(0.05, 0.5, QRN) / sim_times[-1]  # [cm/day]
    soil = loam
    cum_qr = np.zeros(qr_.shape)
    for i, qr in enumerate(qr_):
        cum_qr[i] = solve(soil, sim_times, qr, N)
