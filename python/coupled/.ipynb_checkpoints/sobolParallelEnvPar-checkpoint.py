...
cs = np.array([250,350,500])*1e-6; #example from Dewar2002 [mol mol-1]
TairC = np.array( [0,15,30]); #[°C]
Qlight =np.array([50e-6,250e-6, 500e-6]);#mean absorbed photon irradiance per leaf segment [mol photons m-2 s-1]  
...
def SobolTest(myid,weatherX, X,EnvParam):
    ...
    xXx = np.full((len(r.plant.nodes)),EnvParam[3])
    plant.Qlight= EnvParam[2]
    plant.TairC = EnvParam[1]
    plant.cs = EnvParam[0]
...
for ii in range(maxrun):
    Yvcs = np.array([])
    Yvjs = np.array([])
    Yevs = np.array([])
    EnvParam = np.array([csv[ii],TairCv[ii],Qlightv[ii],meanPsiv[ii]])
    print("I am at maxrun",ii,"of",maxrun,EnvParam)
    didDo = 0
    while didDo < len(param_values):
        ...
        tasks_iterator = (delayed(SobolTest)
                                (i,weatherX, param_values[i],EnvParam)
    ...
    with open('sobolEnv/YAlls_'+repr(ii)+'.pkl','wb') as f:
         pickle.dump({'Yvcs':Yvcs,
                     'Yvjs':Yvjs,
                     'Yevs':Yevs},f, protocol=pickle.HIGHEST_PROTOCOL)
    Yag = np.minimum(Yvcs,Yvjs)

    Sag = sobol.analyze(problem, Yag)
    ...
    with open('sobolEnv/Sag_'+repr(ii)+'.pkl','wb') as f:
         pickle.dump(Sag,f, protocol=pickle.HIGHEST_PROTOCOL)
    with open("sobolEnv/Sag_"+repr(ii)+".txt", 'w') as f: 
        for key, value in Sag.items(): 
            f.write('%s:%s\n' % (key, value))

    with open('sobolEnv/Sic_'+repr(ii)+'.pkl','wb') as f:
         pickle.dump(Sic,f, protocol=pickle.HIGHEST_PROTOCOL)
    with open("sobolEnv/Sic_"+repr(ii)+".txt", 'w') as f: 
        for key, value in Sic.items(): 
            f.write('%s:%s\n' % (key, value))

    with open('sobolEnv/Sij_'+repr(ii)+'.pkl','wb') as f:
         pickle.dump(Sij,f, protocol=pickle.HIGHEST_PROTOCOL)
    with open("sobolEnv/Sij_"+repr(ii)+".txt", 'w') as f: 
        for key, value in Sij.items(): 
            f.write('%s:%s\n' % (key, value))

    with open('sobolEnv/Siev_'+repr(ii)+'.pkl','wb') as f:
         pickle.dump(Siev,f, protocol=pickle.HIGHEST_PROTOCOL)
    with open("sobolEnv/Siev_"+repr(ii)+".txt", 'w') as f: 
