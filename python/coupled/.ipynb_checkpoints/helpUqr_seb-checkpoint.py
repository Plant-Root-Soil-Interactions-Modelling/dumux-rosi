...
def setDefaultVals(r,weatherX,DictVal):
    ...
    r.setRmax_st([[14.4 * DictVal['rr0'],9.0 * DictVal['rr1'],6.0 * DictVal['rr2'],14.4 * DictVal['rr0']],[5. * DictVal['rst'],5. * DictVal['rst']],[15. * DictVal['rle']]])
...
def SobolTest(myid,weatherX, varNames, varLims, simDuration,condition, testType,power2):
    ...
    if testType == "Phloem":
        ...
        directoryN = "./results/sobolSEB/"+str(power2)+condition+str(simDuration)+"/"
    ...
    try:
        ...
        return Yexud,Ygr,Yrm, Ev,Ag,WUE,IWUE, loopsdone
        ...
        with open(name2, 'a') as log:
            ...
            log.write(repr(DictVal) +'\n')
        return 0,0,0,0,0,0,0,plant.loop #np.nan,np.nan,np.nan, np.nan,np.nan,np.nan,np.nan
