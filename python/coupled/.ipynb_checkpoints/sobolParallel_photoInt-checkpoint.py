...
def runallSobol(simDuration, condition):
    ...
    VcmaxrefChl1 = 1.28;#otherwise value too high, original: 1.31
    VcmaxrefChl2 = 8.33; #otherwise value too high, original: 8,52
    ...
    g0 = 8e-3;#residual stomatal opening to CO2, Tuzet 2003 [mol CO2 m-2 s-1]
    ...
    while didDo < len(param_values):
        ...
        tasks_iterator = (delayed(SobolTest)
                                (i,weatherX, param_values[i],simDuration)
    ...
    with open('sobolPhoto4/YAlls_R'+addToName+'.pkl','wb') as f:
    ...
    print();print()
    with open('sobolPhoto4/Siag_R'+addToName+'.pkl','wb') as f:
         pickle.dump(Siag,f, protocol=pickle.HIGHEST_PROTOCOL)
    with open("sobolPhoto4/Siag_R"+addToName+".txt", 'w') as f: 
        for key, value in Siag.items(): 
            f.write('%s:%s\n' % (key, value))

    with open('sobolPhoto4/Sic_R'+addToName+'.pkl','wb') as f:
         pickle.dump(Sic,f, protocol=pickle.HIGHEST_PROTOCOL)
    with open("sobolPhoto4/Sic_R"+addToName+".txt", 'w') as f: 
        for key, value in Sic.items(): 
            f.write('%s:%s\n' % (key, value))

    with open('sobolPhoto4/Sij_R'+addToName+'.pkl','wb') as f:
         pickle.dump(Sij,f, protocol=pickle.HIGHEST_PROTOCOL)
    with open("sobolPhoto4/Sij_R"+addToName+".txt", 'w') as f: 
        for key, value in Sij.items(): 
            f.write('%s:%s\n' % (key, value))

    with open('sobolPhoto4/Siev_R'+addToName+'.pkl','wb') as f:
         pickle.dump(Siev,f, protocol=pickle.HIGHEST_PROTOCOL)
    with open("sobolPhoto4/Siev_R"+addToName+".txt", 'w') as f: 
