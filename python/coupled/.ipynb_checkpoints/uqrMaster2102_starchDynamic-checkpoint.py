...
def launchUQR(directoryN,simInit,simStartSim, condition,csChoise,spellDuration, simEnd):
    ...
    while simDuration < simMax: 
        ...
        Q_Exudmax     = Q_Exud#np.array(r.Q_out[(Nt*7):(Nt*8)])
        ...
        if verbose :
            ...
            print("amount Suc (cm)\n\tAn {:5.2e}\tGr {:5.2e}\tRGr {:5.2e}\tRm {:5.2e}\tExud {:5.2e}".format(AnSum, sum(Q_Gr)*r.Gr_Y,sum(Q_Gr)*(1-r.Gr_Y), sum(Q_Rm), sum(Q_Exud))) 
    ...
    #     fluxes_p = np.insert(fluxes_p,0,0)# "[sucrose]",
        ...
        if dtPrint >= 0.9/24:
            ...
            write_file_array("C_S_ST", C_S_ST)
            write_file_array("C_S_Mesophyll", C_S_Mesophyll)
        ...
        with open('results' + directoryN+'cell2seg.txt', 'a') as f:
            print(r.plant.cell2seg, file=f)
        with open('results' + directoryN+'seg2cell.txt', 'a') as f:
            print(r.plant.seg2cell, file=f)
        cell2segVals = np.concatenate((list(r.plant.cell2seg.values()))).flatten()
        print(len(cell2segVals), len(set(cell2segVals)))
        if len(cell2segVals) != len(set(cell2segVals)):
            print(r.plant.cell2seg, r.plant.seg2cell)
            print(cell2segVals, set(cell2segVals))
            print(len(cell2segVals), len(set(cell2segVals)))
...
if __name__ == '__main__':
    ...
    #simEnd = str(28)
    directoryN = "/"+os.path.basename(__file__)[:-3]+"/"+simInit+"to"+spellStart+"to"+spellEnd+"to"+simEnd+condition+jobID+"/"
    print(directoryN)
