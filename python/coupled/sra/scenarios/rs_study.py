...
ana.addData("SUF", suf)
z_ = np.linspace(0, min_b[2], cell_number[2])  # z - axis
d = ana.distribution("SUF", 0., min_b[2], cell_number[2], False)

print()
print("Krs", krs, "cm2/day")
print("SUF ranges from", np.min(d), "to", np.max(d), "mean", np.mean(d), "sums up to", np.sum(d))

ax[1].plot(d, z_, "-*", label = "total")
...
for i in range(0, max_type + 1):
    ...
    ax[1].plot(d, z_, "-*", label = "type {:g}".format(i))
ax[1].set_title("SUF per layer")
