...
# print(tips)
# print(tips.shape)
gravity = np.zeros((ns, 1))  # we neglect the root collar (N-1)
# gravity[0] = -1 * kx_[0]
# print("first tip at " , tips[0, 1])
# for j in tips[:, 1]:  #
#     seg_index = j - 1
#     i = segments[seg_index].x
#     v = nodes[j,:] - nodes[i,:]
#     gravity[j - 1, 0] = -v[2] * kx_[j - 1]  # v[2]/norm(v) * kx * norm(v)
#     # print(v[2], v[2] * kx_[i - 1])
...
for s in segments:
    ...
    v = nodes[j,:] - nodes[i,:]
    gravity[i - 1, 0] += v[2] * kx_[j - 1]  # v[2]/norm(v) * kx * norm(v)

print("norm", norm(gravity))
print("sum", np.sum(np.abs(gravity) > 1.e-9))

# print("gravity", gravity.shape)
#
# ana = pb.SegmentAnalyser(r.rs)
# ana.addData("gravity", gravity)
# pd = vp.segs_to_polydata(ana, 1., ["radius", "gravity"])
# vp.plot_roots(pd, "gravity")  # axial, radial, rx
...
rx_a2 = (Ainv_dirichlet).dot(Kr.dot(hs)) + Ainv_dirichlet.dot(gravity) + Ainv_dirichlet[:, 0] * kx_[0] * p0  # tried to fix it, not good
...
print("Transpiration (tot)", r.collar_flux(0., rx_a3, [p_s]), "cm3/day")
print("Transpiration (neumann)", r.collar_flux(0., rx_a3, [p_s]), "cm3/day")
