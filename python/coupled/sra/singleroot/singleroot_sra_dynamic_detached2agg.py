...
file1 = 'results/psix_singleroot_sra_dynamicA3_constkrkx' + sstr + '.xls'
df1 = pd.DataFrame(np.transpose(np.array(psi_x_)))
df1.to_excel(file1, index = False, header = False)

file2 = 'results/psiinterface_singleroot_sra_dynamicA3_constkrkx' + sstr + '.xls'
df2 = pd.DataFrame(np.transpose(np.array(psi_s_)))
df2.to_excel(file2, index = False, header = False)

file3 = 'results/sink_singleroot_sra_dynamicA3_constkrkx' + sstr + '.xls'
df3 = pd.DataFrame(-np.transpose(np.array(sink_)))
df3.to_excel(file3, index = False, header = False)

file4 = 'results/transpiration_singleroot_sra_dynamicA3_constkrkx' + sstr
np.savetxt(file4, np.vstack((x_, -np.array(y_))), delimiter = ';')

file5 = 'results/soil_singleroot_sra_dynamicA3_constkrkx' + sstr + '.xls'
