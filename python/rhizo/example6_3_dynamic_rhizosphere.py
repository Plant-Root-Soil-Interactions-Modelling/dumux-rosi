...
from rosi.richards_no_mpi_flat import RichardsNoMPIFlatWrapper  # Python part of 
from rosi.richards_no_mpi import RichardsNoMPIWrapper  # Python part of 
...
def plot_history(area, w, c, n_steps):  # |\label{l63:plot_history_start}|
    ...
    print('c',c.shape,w.shape, area.shape, c_.shape)
    c__ = np.array([sum(cc * area) for cc in c])
    ax1.plot(np.linspace(0, sim_time, n_steps), np.array([sum(cc ) for cc in c]), color="tab:red") #np.sum(np.multiply(area, c), axis=1), color="tab:red")
...
#print('area',area)
plot_history(area,np.array( w), np.array(c), n_steps)  # |\label{l63:plot_history}|
