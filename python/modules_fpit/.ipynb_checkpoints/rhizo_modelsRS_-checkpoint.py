...
class RhizoMappedSegments(pb.MappedRootSystem):#
    """
    ...
    def update(self):
        ...
        self.checkMassOMoleBalance2( sourceWat = np.full(len(sizeSoilCell),0.), # cm3/day 
                                     ...
                                     doWater = True, doSolute = True, doSolid = True,
                                     # useSoilData = True,
                                     diff1d3dCW_abs_lim = max(self.maxDiff1d3dCW_rel[0]*2,self.limErr1d3dAbs) )# error after update should be = error before update
                ...
                try:
                    ...
                    while redoSolve:
                        try:
                            ...
                            if n_iter_solve > 10:
                                raise Exception
                            elif QflowIn_temp < 0 or QflowOut < 0:
                                print(rank, lId,gId,'qflowIn',qIn,' or qflowout',qOut,'too low. Decrease manually the lowest by',2)
                                if QflowIn_temp <= qOut:
                                    QflowIn_temp /=2
                                    qIn = QflowIn_temp/ (2 * np.pi * self.radii[gId] * l) # [cm3/day] -> [cm /day]
                                    cyl.setInnerFluxCyl(qIn) 
                                else:
                                    QflowOut_temp = proposed_outer_fluxes[gId] 
                    ...
                    #QflowIn_limited = (buWAfter - buWBefore)/dt - QflowOut
                    if verbose:
                    ...
                    print( "gId",gId,"l",l,"a_in", self.radii[gId] ,"a_out",self.outer_radii[gId] )
