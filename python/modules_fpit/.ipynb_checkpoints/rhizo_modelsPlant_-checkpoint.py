...
class RhizoMappedSegments(pb.MappedPlant):#XylemFluxPython):#
    ...
    def broadcastPlantShape(self):
        ...
        if rank == 0:
            ...
            airSegs = np.array(list(set(np.concatenate((self.cell2seg.get(-1),
                                                        np.where(np.array(self.organTypes) != 2)[0])) )))
            self.outer_radii[airSegs] = np.array(self.radii)[airSegs]*1.1 #dummy outer radii
            ...
            repartitionSoil = self.getDistribution(len(self.seg_length) - len(airSegs))
            self.toAddSoil = repartitionSoil - self.repartitionSoilOld
            
            repartitionAir = self.getDistribution(len(airSegs))
            ...
            assert sum(repartitionSoil) == len(self.seg_length) - len(airSegs)
            assert sum(self.toAddSoil) == (len(self.seg_length) - len(airSegs) - self.nsSoilOld )
            assert sum(repartitionAir) ==  len(airSegs)
            assert sum(self.toAddAir) == ( len(airSegs) - self.nsAirOld )
            #for next time step
        
        
        airSegs = comm.bcast(airSegs, root = 0) 
        ...
        rhizoSegsId = np.array([i for i in range(len(self.organTypes)) if i not in airSegs])
        newEidxSoil = np.array([])
        if len(newLidxSoil) > 0:
            newEidxSoil = rhizoSegsId[newLidxSoil]
        newEidxAir = np.array([])
        if len(newLidxAir) > 0:
            newEidxAir = airSegs[newLidxAir]
    ...
    def get_concentration(self, compId, konz = True):  
        ...
        isAirSegs = np.array(list(set(np.concatenate((self.cell2seg.get(-1),np.where(np.array(self.organTypes) != 2)[0])) )))#np.array([isinstance(cc , AirSegment) for cc in self.cyls])
        if self.mode.startswith("dumux"):
            if compId == 0:
