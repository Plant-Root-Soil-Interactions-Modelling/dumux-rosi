...
if __name__ == '__main__':
    ...
    results_dir="./results/lowWaterNested_"+str(initsim)+"_"+str(max_rank)+"/"
    if rank == 0:
    ...
    rs, r = scenario.create_mapped_plant(wilting_point, nc, logbase, mode,initsim,
                                            ...
                                            usemoles = usemoles,
                                            limErr1d3d = 5e-9*100)  # pass parameter file for dynamic growth
    ...
    errs2 =np.array(["errRxPlant", "errW1ds","errWrsi","rhizoMassWError_absLim","rhizoMassWError_abs",
                "sum(abs(diffBCS1dsFluxIn))"])
    write_file_array("fpit2_error", errs2, directory_ =results_dir, fileType = '.txt') 
    ...
    errs =np.array(["errRxPlant", "errW1ds", "errW3ds", "max(r.SinkLim3DS)","max(r.SinkLim1DS)","max(r.maxDiff1d3dCW_abs)", 
                ...
                "diff1d3dCurrant","diff1d3d_rel","rhizoMassWError_rel "])
    write_file_array("fpit_error", errs, directory_ =results_dir, fileType = '.txt') 
    ...
    while rs_age < simMax: #for i, dt in enumerate(np.diff(times)):
        ...
        if (mode == "dumux_10c") and (rank == 0) and ((not static_plant) or (rs_age == initsim+dt)):
        ...
        if (rank == 0) and (mode == "dumux_10c")  :
            ...
            print("C_ST (mmol ml-1):\n\tmean {:.2e}\tmin  {:5.2e} at {:d} segs \tmax  {:5.2e}".format(np.mean(C_ST),
                                                                                                      min(C_ST), 
    ...
    if False:
        ...
        for konz in np.array([True]):#, False]):
            ...
            vp.plot_roots_and_soil(rs.mappedSegments(),extraArrayName_,rp, s, periodic, 
                                   min_b, max_b, cell_number, 
                    soil_type+genotype+"_rx", sol_ind =-1,
            ...
            for i in range(1, rs.numComp+1):
                ...
                vp.plot_roots_and_soil(rs.mappedSegments(),extraArrayName_ ,rs.get_concentration(i , konz), 
                                       s, periodic, min_b, max_b, cell_number, 
