...
def simulate_const(s, rs, sim_time, dt, rs_age, Q_plant,
    ...
    for i in range(N):
        ...
        n_iter = 0
        err = 1.e6 
        max_err = 1e-8#r.limErr1d3dAbs/10 # ???
        max_iter = 50 #??
        ...
        while err > max_err and n_iter < max_iter:
            n_iter2 = 0
            err2 = 1.e6
            max_err2 = 1#r.limErr1d3dAbs/100 # ???
            max_iter2 = 100 #??
            while err2 > max_err2 and n_iter2 < max_iter2:
                """ 1. xylem model """
                print('1. xylem model')
                ...
                try:
                    diff_solution0_1ds = np.concatenate(np.array([solution0_1ds_new[i] == solution0_1ds_old[i] 
                                                                  for i in range(len(solution0_1ds_new))],
                                                                 dtype=object))
                ...
                r.solve(dt,max( n_iter2,n_iter),seg_fluxes_limited, proposed_outer_fluxes, seg_sol_fluxes,proposed_outer_sol_fluxes, 
                ...
                rsx = r.get_inner_heads(weather=weatherX)  
                ...
                err2 = comm.bcast(errRxPlant,root = 0)
                errs2 =np.array([errRxPlant, errW1ds, errWrsi, r.rhizoMassWError_absLim,r.rhizoMassWError_abs,
                                sum(abs(diffBCS1dsFluxIn))])
            
                if rank == 0:
                    write_file_array("fpit2_diffBCS1dsFluxIn", diffBCS1dsFluxIn, directory_ =results_dir) 
                    write_file_array("fpit2_diffBCS1dsFluxIn_rhizoSegsId", diffBCS1dsFluxIn[rhizoSegsId], directory_ =results_dir) 

                    if plantType == "plant":
                        TransRate = sum(np.array(rs.Ev)) #transpiration [cm3/day] 
                        write_file_array('fpit2_transRate',np.array([TransRate,TransRate*dt]), directory_ =results_dir )
                        write_file_array("fpit2_errPhoto", np.array([rs.maxErr]) , directory_ =results_dir) 
                        write_file_array("fpit2_errPhotoAbs", np.array([rs.maxErrAbs]) , directory_ =results_dir) 

                    write_file_array("fpit2_error", errs2, directory_ =results_dir) 
                    write_file_array("fpit2_error", errs2, directory_ =results_dir, fileType = '.csv') 
                    write_file_float("fpit2_time", rs_age, directory_ =results_dir) 
                    write_file_array("fpit2_seg_fluxes_limited", seg_fluxes_limited, directory_ =results_dir) 
                    write_file_array("fpit2_seg_fluxes", seg_fluxes, directory_ =results_dir) 
                    write_file_array("fpit2_seg_fluxes", seg_fluxes, directory_ =results_dir, fileType = '.csv') 
                    write_file_array("fpit2_n_iter",np.array([ n_iter2,rs.loop ]), directory_ =results_dir) 
                    write_file_array("fpit2_psi_sri_error", errWrsiAll, directory_ =results_dir) 
                    write_file_array("fpit2_psi_sri_errorRoot", errWrsiAll[rhizoSegsId], directory_ =results_dir) 
                    write_file_array("fpit2_psi_sri_error", errWrsiAll, directory_ =results_dir, fileType = '.csv') 
                    write_file_array("fpit2_psi_sri_errorRoot", errWrsiAll[rhizoSegsId], directory_ =results_dir, fileType = '.csv') 
                    write_file_array("fpit2_psi_sri_real", rsx, directory_ =results_dir) 
                    write_file_array("fpit2_psi_sri_set", rsx_set, directory_ =results_dir) 
                    write_file_array("fpit2_psi_sri_set", rsx_set, directory_ =results_dir, fileType = '.csv') 
                    write_file_array("fpit2_psi_sri_real", rsx, directory_ =results_dir, fileType = '.csv') 
                    write_file_array("fpit2_rhizoWaterPerVoxel", rhizoWaterPerVoxel, directory_ =results_dir) 
                    write_file_array("fpit2_rx", rx, directory_ =results_dir) 
                    write_file_array("fpit2_rhizoWAfter", rhizoWAfter_[rhizoSegsId] , directory_ =results_dir) 
                n_iter2 +=1
            ...
            err = comm.bcast(max(errW3ds,r.rhizoMassWError_rel,errWrsi ),root = 0)
            diff1d3dCurrant = abs(max(r.maxDiff1d3dCW_abs) - maxDiff1d3dCW_absBU)#, root = 0) # to not depend on cumulative error
            ...
            errs =np.array([errRxPlant, errW1ds, errW3ds, 
                            ...
                            diff1d3dCurrant, r.rhizoMassWError_rel ])
        ...
        print('left iteration', rank, n_iter, err, max(r.maxDiff1d3dCW_abs))
        if rank == 0:
