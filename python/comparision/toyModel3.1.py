...
s.setParameter("Soil.alpha", str(1*0)) #[1/d]
...
CSS2_init = 0#CSSmax*1e6 *(C_S/(C_S+ k_sorp*1e6)) * (1 - f_sorp)/2 #mol C/ m3 scv
...
print(contents0)
# print(s.getSolution_(1).flatten()*molarDensityWat*1e6)
...
for i, dt in enumerate(np.diff(times)):
    ...
    s.solve(dt, maxDt = 10)#2500/(24*3600))
    ...
    cstot_cls = (np.array(s.getWaterContent()).flatten() 
                * C_S_W) #* points
    cstot_css1 =  CSSmax*1e6 * (C_S_W/(C_S_W+k_sorp*1e6)) * f_sorp  #* points
