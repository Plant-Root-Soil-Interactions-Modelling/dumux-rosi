...
Parameters  
"""
name = "CStudy_Sand_dry"  # scenario name, to save results 

""" soil """
min_b = [-4., -4., -15.]  # cm
max_b = [4., 4., 0.]  # cm
domain_volume = np.prod(np.array(max_b) - np.array(min_b))
cell_number = [8, 8, 15]  # [8, 8, 15]  # [16, 16, 30]  # [32, 32, 60]  # [8, 8, 15] # [1]
periodic = False
...
# for post processing 
min_sx, min_rx, min_rsx, collar_sx, collar_flux, ESWP, mean_bulk_soil_pressure , ESWP_check, mean_xylem_pressure, mean_RSI_pressure, root_collar_pressure, mean_soil_k, rsx_new_mean = [], [], [], [], [], [], [] , [] , [] , [] , [] , [], []# cm
...
for i in range(0, NT):
    ...
    #soil_k = kr2 # UNBEDINGT ZURUECKAENDERN, DAS IST NUR EIN TEST!!! np.divide(vg.hydraulic_conductivity(rsx, soil), rs.radii)  
    soil_k = np.divide(vg.hydraulic_conductivity(rsx, soil), rs.radii) # only valid for homogenous soil
    #print(rs.radii)
    print(np.mean(vg.hydraulic_conductivity(rsx, soil)))
    if rank == 0:
    ...
    if i % skip == 0:
        ...
        mean_bulk_soil_pressure.append(np.mean(s.getSolutionHead()))
        ...
        if rank == 0: 
            ...
            """ Equivalent soil water potential (Tobi)""" 

            # Reverse regularisation with min(kr, k_soil) for comparable ESWP values
            #print("len rx", len(rx))
            #print("len fluxes=", len(realized_inner_fluxes))
            #print("len rx slice", len(rx[1:])) # slice off first value of rx list (i guess this is the collar value, and there is no realized inner flux to this?)
            #print("collar value? =", rx[0]) # => yes, collar value! => c
            #print(rx)
            #print(rx[:-1])
            #print(rx+rx[:-1])
            rsx_new = realized_inner_fluxes / kr + rx[1:]
            #print(rsx_new)
            #print(len(rsx_new))
            rsx_new_mean.append(np.mean(rsx_new))
            #eswp = r.get_eswp(t, rsx_new)
            #ESWP.append(float(eswp))
            
            mean_RSI_pressure.append(np.mean(rsx))
            min_rx.append(np.min(np.array(rx)))
            root_collar_pressure.append(min_rx[-1])
            ...
            mean_soil_k.append(np.mean(soil_k))
            '''
            print("mean bulk soil pressure         " + str(mean_bulk_soil_pressure[i]))
            print("ESWP                            " + str(ESWP[-1]))
            print("mean RSI pressure old           " + str(mean_RSI_pressure[-1]))
            print("mean RSI pressure new           " + str(rsx_new_mean[-1]))
            print("mean xylem pressure             " + str(mean_xylem_pressure[-1]) + "   (still based on regularized kr!)")
            print("root_collar_pressure            " + str(root_collar_pressure[-1]))
            print("kr= " + str(kr) +"      soil_k= " + str(mean_soil_k[i]))
            '''
...
if rank == 0:
    ...
    WCT= round(timeit.default_timer() - start_time)
    ...
    np.savetxt(name + "_transpiration", np.vstack((out_times, -np.array(collar_flux), -np.array(water_uptake), np.cumsum(-np.array(water_uptake) * dt))), delimiter=';')

    fig, ax1 = plt.subplots()
    ax1.plot(out_times, mean_bulk_soil_pressure, "chocolate")
    ax1.plot(out_times, ESWP, "b--")
    ax1.plot(out_times, rsx_new_mean, "r--")
    ax1.plot(out_times, root_collar_pressure, "g")
    #ax1.plot(out_times, rsx_new_mean)
    #ax1.plot(x_, cpx, "g")
    ...
    fig.suptitle(name + "  WCT= " + str(WCT)+"s", )#y=1)
    ...
    # soil_k_plot
    fig, ax1 = plt.subplots()
    ax1.plot(out_times, mean_soil_k, "chocolate")
    ax1.plot([0,10], [kr,kr], "r")
    #ax1.plot(x_, cpx, "g")
    #ax1.plot(x_, min_bulk_soil_pressure)
    ax1.set_xlabel("Time [d]")
    ax1.set_ylabel("soil pressure [cm]")
    ax1.legend(["mean soil conductivity (of voxels around segments)", "kr", ], loc = "upper right") 
    fig.suptitle(name + "  WCT= " + str(WCT)+"s", )#y=1)
    fig.set_tight_layout(True)
    fig.savefig("conductivities")
    np.savetxt(name + "_pressures", np.vstack((out_times, mean_bulk_soil_pressure, ESWP, root_collar_pressure)), delimiter = ';')
    plt.show()
