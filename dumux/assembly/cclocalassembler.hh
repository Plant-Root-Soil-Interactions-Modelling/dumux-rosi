            ...
            CCLocalAssembler<TypeTag, Assembler, DiffMethod::analytic, true>, true>
{
    ...
    enum { numEq1 = 1};//GetPropType<TypeTag, Properties::ModelTraits>::numEq() };
...
public:
    ...
    NumEqVector assembleJacobianAndResidualImpl(JacobianMatrix& A, GridVariables& gridVariables)
    {
        ...
        if (this->elementIsGhost())
        {
			...
			// auto origResiduals0 = assembleJacobianAndResidualImpl2(A, gridVariables);
            // return zero residual
            return NumEqVector(0.0);
        }

        // assemble the undeflected residual
        auto residual = this->evalLocalResidual()[0]; // 0: owner cell of the element?
        ...
        if constexpr (Problem::enableInternalDirichletConstraints())
        {
            ...
            for (int pvIdx = 0; pvIdx < numEq1; ++pvIdx)
            {
		...
		auto origResiduals0 = assembleJacobianAndResidualImpl2(A, gridVariables);
		for (int eqIdx = numEq1; eqIdx < numEq; eqIdx++)
		{
			residual[eqIdx] = origResiduals0[eqIdx];
		}
		if(this->problem().verbose_assembler)
		{
			for (int row = 0; row < numEq; ++row)
			{
				for (int col = 0; col < numEq; ++col)
				{
					std::cout << "A[" << globalI << "][" << globalI << "]["
							  << row << "][" << col << "] = "
							  << A[globalI][globalI][row][col]
							  << std::endl;
				}
			}
			...
			const auto& connectivityMap = this->assembler().gridGeometry().connectivityMap();
			...
			for (int eq = 0; eq < numEq; ++eq)
			{
				...
				for (int pv = 0; pv < numEq; ++pv)
					if (std::abs(A[globalI][globalI][eq][pv]) > 1e-20)
						++nnz;

				// neighbor blocks
				for (const auto& dataJ : connectivityMap[globalI])
					for (int pv = 0; pv < numEq; ++pv)
						if (std::abs(A[dataJ.globalJ][globalI][eq][pv]) > 1e-20)
							++nnz;

				std::cout << "Equation " << eq << " : nnz = " << nnz << '\n';
			}
		}
     ...
     * \return The element residual at the current solution.
     */
    NumEqVector assembleJacobianAndResidualImpl2(JacobianMatrix& A, GridVariables& gridVariables)
    {
        //////////////////////////////////////////////////////////////////////////////////////////////////
        // Calculate derivatives of all dofs in stencil with respect to the dofs in the element. In the //
        // neighboring elements we do so by computing the derivatives of the fluxes which depend on the //
        ...
        // Perturb ALL primary variables (water + solute) numerically,
		// but only scatter derivatives into the SOLUTE rows of A.
		// Row 0 (water) was already assembled analytically above and is
		// never touched here — dRw/dw is already correct, and dRw/ds = 0
		// because water flow doesn't depend on solute.
		for (int pvIdx = 0; pvIdx < numEq; ++pvIdx)
		{
			partialDerivs = 0.0;
			...
			// ============= debug print #1 (from the original water/solute loop)
			if (this->problem().verbose_assembler)
			{
				auto restoredResidual = this->evalLocalResidual()[0];

				std::cout << "Restored residual after pv "
						  << pvIdx << " : ";

				for (int eq = 0; eq < numEq; ++eq)
					std::cout << restoredResidual[eq] << " ";

				std::cout << '\n';
			}
			// ============= debug print #1 done

			// fill ONLY the solute rows
			for (int eqIdx = numEq1; eqIdx < numEq; ++eqIdx)
			{
				A[globalI][globalI][eqIdx][pvIdx] += partialDerivs[0][eqIdx];
			...
			// ============= debug print #2 (from the water-pv fill loop)
			if (this->problem().verbose_assembler)
			{
				auto testResidual = this->evalLocalResidual()[0];

				std::cout << "Residual after restoring, pv=" << pvIdx << '\n';
				for (int eq = 0; eq < numEq; ++eq)
					std::cout << eq << " : " << testResidual[eq] << '\n';
			}
			// ============= debug print #2 done
		}
		// ============= Added-done
