...
namespace Dumux {
    ...
    public:
        ...
        InputFileFunction(std::string nameY, std::string nameX, int dataIdx,
                          int typeIdx = 0, bool enablePeriodic = false, InputFileFunction* typeF = nullptr) {
            dataIdx_ = dataIdx;
            typeIdx_ = typeIdx;
            try {
        ...
        InputFileFunction(std::string nameY, InputFileFunction* typeF, int typeIdx = 0) {
            typeIdx_ = typeIdx;
            try {
                    yy_ = Dumux::getParam<std::vector<double>>(nameY);
                    if (yy_.size() == 1) {
