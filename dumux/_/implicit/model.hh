...
public:
    ...
    {
        res.template deserializeEntities<dofCodim>(asImp_(), this->gridView_(), restartSolutions);
        prevSol() = curSol();
    }
    /*!
    ...
    template <class Entity>
    void deserializeEntity(std::vector<Scalar> restartSolutions,
                           const Entity &entity)
    {
        int dofIdxGlobal = dofMapper().index(entity);

        for (int eqIdx = 0; eqIdx < numEq; ++eqIdx) {
            curSol()[dofIdxGlobal][eqIdx] = restartSolutions[eqIdx];
        }
    }
