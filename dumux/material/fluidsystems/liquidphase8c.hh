 ...
 * \brief @copybrief Dumux::FluidSystems::LiquidPhaseEightC
 */
#ifndef DUMUX_LIQUID_EIGHTC_PHASE_HH
#define DUMUX_LIQUID_EIGHTC_PHASE_HH
...
template <class Scalar, class MainComponent, class SecondComponent, class ThirdComponent,
			class FourthComponent, class FifthComponent, class SixthComponent,  class SeventhComponent, 
			class EighthComponent,
			class Policy = liquidp8cDefaultPolicy<>>
class LiquidPhaseEightC
: public Base<Scalar, LiquidPhaseEightC<Scalar, MainComponent, SecondComponent, ThirdComponent,
			FourthComponent, FifthComponent, SixthComponent,  SeventhComponent,EighthComponent> >
{
    using ThisType = LiquidPhaseEightC<Scalar, MainComponent, SecondComponent, ThirdComponent,
					FourthComponent, FifthComponent, SixthComponent,  SeventhComponent,EighthComponent, Policy>;
    ...
    using BinaryCoefficientsFourthComponent = BinaryCoeff::H2O_Component<Scalar, FourthComponent>;
    using BinaryCoefficientsFifthComponent  = BinaryCoeff::H2O_Component<Scalar, FifthComponent>;
    using BinaryCoefficientsSixthComponent = BinaryCoeff::H2O_Component<Scalar, SixthComponent>;
    using BinaryCoefficientsSeventhComponent  = BinaryCoeff::H2O_Component<Scalar, SeventhComponent>;
    using BinaryCoefficientsEighthComponent  = BinaryCoeff::H2O_Component<Scalar, EighthComponent>;

public:
    using ParameterCache = NullParameterCache;

    static constexpr int numPhases = 1; //!< Number of phases in the fluid system
    static constexpr int numComponents = 8; //!< Number of components in the fluid system
    ...
    static constexpr int comp3Idx = 3; //!< index of the 3rd component
    static constexpr int comp4Idx = 4; //!< index of the 3rd component
    static constexpr int comp5Idx = 5; //!< index of the 3rd component
    static constexpr int comp6Idx = 6; //!< index of the 3rd component
    static constexpr int comp7Idx = 7; //!< index of the 3rd component
    ...
    static constexpr int fourthCompIdx = comp3Idx; //!< index of the secondary component
    static constexpr int fifthCompIdx = comp4Idx; //!< index of the secondary component
    static constexpr int sixthCompIdx = comp5Idx; //!< index of the secondary component
    static constexpr int seventhCompIdx = comp6Idx; //!< index of the secondary component
    static constexpr int eighthCompIdx = comp7Idx; //!< index of the secondary component
    ...
    */
    ...
    { 
		...
		switch(compIdx)
		{
			...
			default:
			{
				DUNE_THROW(Dune::InvalidStateException, "liquidphase8c::componentName: compIdx "<<compIdx<<" not recognised");
			}				
		}		
    ...
    {  
		...
		switch(compIdx)
		{
			...
			default:
			{
				DUNE_THROW(Dune::InvalidStateException, "liquidphase8c::molarMass: compIdx "<<compIdx<<" not recognised");
			}
		}		
		return molarMass_; 
	}
                          ...
                          const int phaseIdx = 0)
    {
		...
		// assume pure water
            ...
            {
				DUNE_THROW(Dune::InvalidStateException, "liquidphase8c::density: not useH2ODensityAsLiquidMixtureDensity");
			}
    ...
    */
                                    ...
                                    int componentIdx)
    {
        ...
        else if (componentIdx == fourthCompIdx)
            return FourthComponent::liquidEnthalpy(T, p);
        else if (componentIdx == fifthCompIdx)
            return FifthComponent::liquidEnthalpy(T, p);
        else if (componentIdx == sixthCompIdx)
            return SixthComponent::liquidEnthalpy(T, p);
        else if (componentIdx == seventhCompIdx)
            return SeventhComponent::liquidEnthalpy(T, p);
        else if (componentIdx == eighthCompIdx)
            return EighthComponent::liquidEnthalpy(T, p);
        else
    ...
    static Scalar binaryDiffusionCoefficient(const FluidState &fluidState, int phaseIdx, int compIIdx, int compJIdx)
    {
		...
		switch(compJIdx) {
		  case secondCompIdx:
			...
			case fourthCompIdx:
				return BinaryCoefficientsFourthComponent::liquidDiffCoeff(fluidState.temperature(phaseIdx), fluidState.pressure(phaseIdx));
			case fifthCompIdx:
				return BinaryCoefficientsFifthComponent::liquidDiffCoeff(fluidState.temperature(phaseIdx), fluidState.pressure(phaseIdx));
			case sixthCompIdx:
				return BinaryCoefficientsSixthComponent::liquidDiffCoeff(fluidState.temperature(phaseIdx), fluidState.pressure(phaseIdx));
			case seventhCompIdx:
				return BinaryCoefficientsSeventhComponent::liquidDiffCoeff(fluidState.temperature(phaseIdx), fluidState.pressure(phaseIdx));
			case eighthCompIdx:
				return BinaryCoefficientsEighthComponent::liquidDiffCoeff(fluidState.temperature(phaseIdx), fluidState.pressure(phaseIdx));
			default:
				DUNE_THROW(Dune::InvalidStateException, "Invalid component index " << compJIdx);
		}
        return -1;
    }
