 ...
 */
#ifndef DUMUX_MY_CC_FV_ELEMENTGEOMETRY_HH
#define DUMUX_MY_CC_FV_ELEMENTGEOMETRY_HH
...
public:
    ...
    void update(const GridView& gridView, const Element& element, const Problem& problem)
    {
        ...
        // push the neighbors of the circle stencil if they exist
        const auto& stencil = problem.couplingManager().stencil(element);
        if (stencil.size() > numNeighbors)
        {
            for (int i = numNeighbors; i < stencil.size(); ++i)
            {
                neighbors.push_back(problem.boundingBoxTree().entity(stencil[i]));
                numNeighbors++;
            }
        }
