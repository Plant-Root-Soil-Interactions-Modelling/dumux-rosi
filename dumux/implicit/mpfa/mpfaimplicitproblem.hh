 ...
 */
#ifndef DUMUX_MPFA_IMPLICIT_PROBLEM_HH
#define DUMUX_MPFA_IMPLICIT_PROBLEM_HH
...
private:
    ...
    MpfaImplicitProblem(const MpfaImplicitProblem &);
...
public:
     ...
     */
    MpfaImplicitProblem(TimeManager &timeManager, const GridView &gridView)
        : gridView_(gridView)
    ...
    const int isInternalBoundary(const Intersection &intersection) const
    {
        return asImp_().isInternalBoundaryAtPos(intersection.geometry().center());
    }
    // if method is not overloaded, return an interior face as a default
    const int isInternalBoundaryAtPos(const GlobalPosition &globalPos) const
    {
        return BoundaryInteractionVolume::FaceTypes::InteriorFace;
    }

    void internalDirichlet(PrimaryVariables &values,
                            const Element &element,
                            const int facetIdxOnElement,
                            const BoundaryInteractionVolume &interactionVolume) const
    {
        values = 0;

        if ( GET_PROP_VALUE(TypeTag, FacetCoupling) )
            DUNE_THROW(Dune::InvalidStateException, "Facet coupling is active, "
                                "but no appropriate internalDirichlet routine provided!");

        // forward it to the method which only takes the global coordinate
        asImp_().internalDirichletAtPos(values, element.template subEntity<1>(facetIdxOnElement).geometry().center());
    }

    void internalDirichletAtPos(PrimaryVariables &values,
                        const GlobalPosition &globalPos) const
    {
        DUNE_THROW(Dune::InvalidStateException,
                   "The problem does not provide "
                   "a internalDirichletAtPos() method.");
    }

    // localVertIdx is index of vertex w.r.t. element
    void internalFlux(PrimaryVariables &values,
                      const Element &element,
                      const int facetIdxOnElement,
                      const BoundaryInteractionVolume &interactionVolume) const
    {
        ...
        int elementGlobalIdx = this->elementMapper().index(element);

        int subVolIdx = interactionVolume.findSubVolumeIndex(elementGlobalIdx);
        int localFaceIndex = interactionVolume.getFacetIndexOnSubVolume(subVolIdx, facetIdxOnElement);
        GlobalPosition globalPos = interactionVolume.getSubVolume(subVolIdx).contiPoints[localFaceIndex];

        // forward it to the method which only takes the global coordinate
        asImp_().internalFluxAtPos(values, globalPos);
    }

    void internalFluxAtPos(PrimaryVariables &values,
                        const GlobalPosition &globalPos) const
    {
        DUNE_THROW(Dune::InvalidStateException,
                   "The problem does not provide "
                   "a internalFluxAtPos() method.");
    }

    void getVolumeVarsOnFacet(VolumeVariables &values,
                            const Element &element,
                            const int facetIdxOnElement,
                            const BoundaryInteractionVolume &interactionVolume) const
    {
         DUNE_THROW(Dune::InvalidStateException,
                   "Facet coupling is active, "
                                "but no appropriate getVolumeVarsOnFacet routine provided!");
    }

    // Dummy methods for models without facet coupling
    Scalar getLowDimPermeability(const Element &element, int facetIdxOnElement) const
    {
        if ( GET_PROP_VALUE(TypeTag, FacetCoupling) )
            DUNE_THROW(Dune::InvalidStateException,
                   "Facet coupling is active, but no appropriate getLowDimPermeability routine provided!");
        else
            return 0;
    }
    Scalar getLowDimDomainAperture(const Element &element, int facetIdxOnElement) const
    {
        if ( GET_PROP_VALUE(TypeTag, FacetCoupling) )
            DUNE_THROW(Dune::InvalidStateException,
                   "Facet coupling is active, but no appropriate getLowDimDomainDimension routine provided!");
        else
            return 1;
    }
                 ...
                 const BoundaryInteractionVolume &interactionVolume) const
    {
        int elementGlobalIdx = this->elementMapper().index(element);
        int subVolIdx = interactionVolume.findSubVolumeIndex(elementGlobalIdx);
        int localFaceIndex = interactionVolume.getFacetIndexOnSubVolume(subVolIdx, intersection.indexInInside());
        GlobalPosition globalPos = interactionVolume.getSubVolume(subVolIdx).contiPoints[localFaceIndex];

        // forward it to the interface with only the global position
        asImp_().neumannAtPos(values, globalPos);
    }
