 ...
 */
#ifndef DUMUX_MPFA_IMPLICIT_MODEL_HH
#define DUMUX_MPFA_IMPLICIT_MODEL_HH
...
class MpfaImplicitModel
{
    ...
    typedef typename GET_PROP_TYPE(TypeTag, MpfaBoundaryInteractionVolume) MpfaBoundaryInteractionVolume;
    ...
    MpfaImplicitModel(const MpfaImplicitModel &);
...
public:
    ...
    {
        if (isBox)
            DUNE_THROW(Dune::InvalidStateException, "Mpfa implementation requires property \"isBox\" to be false!");

        problemPtr_ = &problem;

        int mpfaMethod = GET_PROP_VALUE(TypeTag, MpfaMethod);

        std::cout << "initializing interaction volumes for ";
        if (mpfaMethod == GET_PROP(TypeTag, MpfaMethods)::oMethod)
            std::cout << "the Mpfa-O method...";
        else if (mpfaMethod == GET_PROP(TypeTag, MpfaMethods)::lMethod)
        {
            std::cout << "the Mpfa-l method ";
            if (GET_PROP_VALUE(TypeTag, FullMpfaOBoundary))
                std::cout << "with full mpfa-O boundary treatment...";
            else
                std::cout << "with half mpfa-O boundary treatment...";
        }
        interactionVolumeContainer_.init(problem);
    ...
    // Pass diffusion coefficient for the calculation of the transmissivities
    // in the interaction volumes. By default, the intrinsic permeability is forwarded.
    const WorldMatrix getDiffusionCoefficient(const Element &element) const
    {
        return problem_().spatialParams().intrinsicPermeabilityAtPos(element.geometry().center());
    }
    ...
    std::shared_ptr<InteractionVolume> getInnerInteractionVolume(const Element& element, int facetIdxOnElement, int localVertIdx)
    {
        return interactionVolumeContainer_.getInnerInteractionVolume(element, facetIdxOnElement, localVertIdx);
    }
    ...
    std::shared_ptr<MpfaBoundaryInteractionVolume> getBoundaryInteractionVolume(const Element &element, int facetIdxOnElement, int localVertIdx, int phaseIdx)
    {
        return interactionVolumeContainer_.getBoundaryInteractionVolume(element, facetIdxOnElement, localVertIdx, phaseIdx);
    }

    std::shared_ptr<MpfaBoundaryInteractionVolume> getBoundaryInteractionVolume(const Element &element, int facetIdxOnElement, int localVertIdx, int phaseIdx) const
    {
        return interactionVolumeContainer_.getBoundaryInteractionVolume(element, facetIdxOnElement, localVertIdx, phaseIdx);
    }

    std::shared_ptr<MpfaBoundaryInteractionVolume> getInsideInteractionVolume(const Element &element, int facetIdxOnElement, int localVertIdx)
    {
        return interactionVolumeContainer_.getInsideInteractionVolume(element, facetIdxOnElement, localVertIdx);
    }

    std::shared_ptr<MpfaBoundaryInteractionVolume> getInsideInteractionVolume(const Element &element, int facetIdxOnElement, int localVertIdx) const
    {
        return interactionVolumeContainer_.getInsideInteractionVolume(element, facetIdxOnElement, localVertIdx);
    }

    const InteractionVolumeContainer &interactionVolumeContainer() const
    {
        return interactionVolumeContainer_;
    }
...
protected:
    ...
    void applyInitialSolution_()
    {
        ...
        for (; eIt != eEndIt; ++eIt) {
            ...
            fvGeometry.update(gridView_(), *eIt, problem_());
...
private:
    ...
    InteractionVolumeContainer interactionVolumeContainer_;
};
} // end namespace Dumux
