...
public:
        ...
        for (int phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx)
        {
            for (int compIdx = 0; compIdx < numComponents; ++compIdx)
            {
                ...
                if (eqIdx != replaceCompEqIdx) {
                    storage[eqIdx] += (volVars.porosity()*volVars.saturation(phaseIdx)+b)
                                      ...
                                      * massOrMoleFraction(volVars, phaseIdx, compIdx) * pos0 ;
                }
            }
            ...
            if (useTotalMoleOrMassBalance) {
                storage[replaceCompEqIdx] += massOrMoleDensity(volVars, phaseIdx)
                                             *  (volVars.porosity()*volVars.saturation(phaseIdx)) * pos0;
            }
                            ...
                            const ElementFluxVariablesCache& elemFluxVarsCache) const
    {
        ...
        for (int phaseIdx = 0; phaseIdx < numPhases; ++phaseIdx)
        {
            const auto diffusiveFluxes = fluxVars.molecularDiffusionFlux(phaseIdx) * pos0;
            for (int compIdx = 0; compIdx < numComponents; ++compIdx)
            {
                // get equation index
