 ...
 *
 */
#ifndef DUMUX_RICHARDS_2C_BUFFER_RADIALLY_SYMMETRIC_FLUX_VARIABLES_HH
#define DUMUX_RICHARDS_2C_BUFFER_RADIALLY_SYMMETRIC_FLUX_VARIABLES_HH
...
class RichardsTwoCBufferRadiallySymmetricFluxVariables
{
...
public:
    ...
    RichardsTwoCBufferRadiallySymmetricFluxVariables(const Problem &problem,
     ...
     */
    RichardsTwoCBufferRadiallySymmetricFluxVariables() = default;
                ...
                const bool onBoundary = false)
    {
        ...
        density_ = 0.0;
        potentialGrad_ = 0.0;
        radialPotentialGrad_ = 0.0;
        moleFractionGrad_ = 0.0;
        massFractionGrad_ = 0.0;
        concentrationGrad_ = 0.0;
        radialConcentrationGrad_ = 0.0;
        coordinatesCenter_=element.geometry().center();
    ...
    const GlobalPosition coordinatesCenter() const
    { return coordinatesCenter_; }
    /*!
...
/*!
     * \brief Return the pressure potential gradient \f$\mathrm{[Pa/m]}\f$.
     */
    const GlobalPosition &radialPotentialGrad() const
    { return radialPotentialGrad_; }
    ...
    const GlobalPosition &radialConcentrationGrad(int compIdx) const
    {
       if (compIdx != 1)
       { DUNE_THROW(Dune::InvalidStateException,
                "The 1p2c model is supposed to need "
                "only the concentration gradient of "
                "the second component!"); }
       return radialConcentrationGrad_;
    };
    ...
    */
     ...
     * \param phaseIdx index of the phase
     */
    Scalar radialVolumeFlux(const unsigned int phaseIdx) const
    {
        assert (phaseIdx == Indices::phaseIdx);
        return radialVolumeFlux_;
    }
protected:
...
//
    ...
    */
    ...
    //----------------------------------------------
        K_.mv(radialPotentialGrad_, Kmvrp_);
        KmvrpNormal_ = -(Kmvrp_*face().normal);
        ...
        radialVolumeFlux_ = KmvrpNormal_;
        radialVolumeFlux_ *= mobilityUpwindWeight_/elemVolVars[upstreamIdx_].viscosity()
                    + (1.0 - mobilityUpwindWeight_)/elemVolVars[downstreamIdx_].viscosity();
    //----------
        radialVolumeFlux_ = problem.spatialParams().innerFlux()*problem.bBoxMax()[0]/element.geometry().center();
        volumeFlux_ = problem.spatialParams().innerFlux();
    }
    /*!
    ...
    */
                                    ...
                                    const ElementVolumeVariables &elemVolVars)
    {
        ...
        Valgrind::CheckDefined(radialPotentialGrad());
        Valgrind::CheckDefined(K_);
        //K_.mv(potentialGrad(), velocity);
        K_.mv(radialPotentialGrad(), velocity);
    ...
    GlobalPosition radialPotentialGrad_;
    ...
    GlobalPosition radialConcentrationGrad_;
    GlobalPosition coordinatesCenter_;
