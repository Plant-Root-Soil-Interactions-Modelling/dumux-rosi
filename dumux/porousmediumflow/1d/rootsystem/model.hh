...
#ifndef DUMUX_ROOTSYSTEM_MODEL_HH
#define DUMUX_ROOTSYSTEM_MODEL_HH
...
class RootsystemModel : public GET_PROP_TYPE(TypeTag, BaseModel)
{
...
public:
                            ...
                            MultiWriter &writer)
    {
        ...
        for (const auto& element : elements(this->gridView_()))
        {
            ...
            {
                int globalIdx = this->dofMapper().subIndex(element, dofCodim, scvIdx);

                (*p)[globalIdx] = elemVolVars[scvIdx].pressure();
                (*ph)[globalIdx] = elemVolVars[scvIdx].pressureHead(0);
                (*rootRadius)[globalIdx] = spatialParams.rootRadius(element, fvGeometry, scvIdx);
                (*rootSurface)[globalIdx] = spatialParams.rootSurface(element, fvGeometry, scvIdx);
                (*rootBranch)[globalIdx] = spatialParams.rootBranch(element, fvGeometry, scvIdx);
                (*rootOrder)[globalIdx] = spatialParams.rootOrder(element, fvGeometry, scvIdx);
                (*rootMass)[globalIdx] = spatialParams.rootMass(element, fvGeometry, scvIdx);
                (*Kx)[globalIdx] = spatialParams.Kx(element, fvGeometry, scvIdx);
                (*Kr)[globalIdx] = spatialParams.Kr(element, fvGeometry, scvIdx);
        ...
        writer.attachDofData(*Kx, "Kx", isBox);
        writer.attachDofData(*Kr, "Kr", isBox);
