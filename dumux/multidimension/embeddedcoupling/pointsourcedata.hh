...
public:
    ...
    void interpolateBulk(const BulkSolutionVector& sol)
    {
        if (bulkIsBox)
        {
            bulkPriVars_ = 0.0;
        ...
        {
            bulkPriVars_ = sol[bulkElementIdx()];
        }
    }

    void interpolateLowDim(const LowDimSolutionVector& sol)
    {
        if (lowDimIsBox)
        {
            lowDimPriVars_ = 0.0;
        ...
        {
            //std::cout<<" interpolateLowDim sol.lowDimElementIdx !  \n";
            lowDimPriVars_ = sol[lowDimElementIdx()];
        }
    }

    const BulkPrimaryVariables& bulkPriVars() const
    { return bulkPriVars_; }

    const LowDimPrimaryVariables& lowDimPriVars() const
    { return lowDimPriVars_; }

    void setBulkPriVars(const BulkPrimaryVariables& priVars)
    { bulkPriVars_ = priVars; }

    void setLowDimPriVars(const LowDimPrimaryVariables& priVars)
    { lowDimPriVars_ = priVars; }
...
public:
    ...
    void interpolateBulk(const BulkSolutionVector& sol)
    {
        ...
        if (enableBulkCircleInterpolation_)
        {
            ...
            this->setBulkPriVars(std::move(bulkPriVars));
        }
        else
        {
            ParentType::interpolateBulk(sol);
        }
    }
