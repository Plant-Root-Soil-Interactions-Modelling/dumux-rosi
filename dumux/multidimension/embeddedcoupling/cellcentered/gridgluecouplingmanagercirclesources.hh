...
#ifndef DUMUX_CC_GRIDGLUE_EMBEDDEDCOUPLINGMANAGER_CIRCLESOURCES_HH
#define DUMUX_CC_GRIDGLUE_EMBEDDEDCOUPLINGMANAGER_CIRCLESOURCES_HH
...
class CCGridGlueEmbeddedCouplingManagerCircleSources : public CCGridGlueEmbeddedCouplingManager<TypeTag>
{
...
public:
     ...
     * \brief Constructor
     */
    CCGridGlueEmbeddedCouplingManagerCircleSources(BulkProblem& bulkProblem, LowDimProblem& lowDimProblem)
    ...
    void computePointSourceData(unsigned int order = 1, bool verbose = false)
    {
        ...
        Dune::Timer watch;
        std::cout << "Initializing the integration point sources..." << std::endl;
        ...
        for (auto isIt = glue.template ibegin<1>(); isIt != isEndIt; ++isIt)
        {
            ...
            for (auto&& qp : quad)
            {
                ...
                // writer.write("circle_" + std::to_string(circleCounter++));
                ...
                auto lowDimDescriptor = [](const LowDimElement& element, unsigned int subentity) { return true; };
                auto circleExtractor = std::make_shared<LowDimExtractor>(circleGrid->leafGridView(), lowDimDescriptor);
                auto bulkExtractor = std::make_shared<BulkExtractor>(this->bulkGridView(), bulkDescriptor);
                auto circleMerger = std::make_shared<Dune::GridGlue::OverlappingMerge<bulkDim, lowDimDim, dimWorld>>();
                GlueType circleGlue(bulkExtractor, circleExtractor, circleMerger);

                // enable fallback brute-force step if the advancing front algorithm didn't find a seed
                circleMerger->enableFallback(true);
                ...
                {
                    // use trapezoidal rule for the source term computation for the lowdim element
                    ...
                    // for the coupling from low dim to bulk elements create a point source
                    // at each quadrature point of the circle intersection
                    for (auto&& qpCircle : quad)
                    {
                        // get global position of quadrature point
                        const auto globalPosCircle = cirlceIntersectionGeometry.global(qpCircle.position());
                        const auto integrationElementCircle = cirlceIntersectionGeometry.integrationElement(qpCircle.position());

                        for (unsigned int outsideIdx = 0; outsideIdx < circleIsIt->neighbor(0); ++outsideIdx)
                        {
                            const auto& outside = circleIsIt->outside(outsideIdx);
                            const unsigned int bulkElementIdx = this->bulkProblem().elementMapper().index(outside);

                            const auto id = idCounter_++;
                            const auto qpCircleweight = qpCircle.weight() * qp.weight(); // double integration
                            const auto ie = integrationElement * integrationElementCircle; // double integration
                            this->bulkPointSources().emplace_back(globalPosCircle, id, qpCircleweight, ie, std::vector<unsigned int>({bulkElementIdx}));
                            this->bulkPointSources().back().setEmbeddings(circleIsIt->neighbor(0));

                            // point source data related to the bulk circle sources
                            // these data don't contain any special circle interpolation
                ...
                // compute the coupling point sources for coupling from bulk to lowdim
                for (unsigned int outsideIdx = 0; outsideIdx < isIt->neighbor(0); ++outsideIdx)
                {
                    ...
                    this->lowDimPointSources().emplace_back(globalPos, id, qpweight, integrationElement, std::vector<unsigned int>({lowDimElementIdx}));
