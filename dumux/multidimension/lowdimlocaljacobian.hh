...
/*!
 * \ingroup MultiDimension
 * \brief Calculates the Jacobian of the local residual for the low-dimensional domain
 *
...
class LowDimLocalJacobian
{
private:
    typedef typename GET_PROP_TYPE(TypeTag, LowDimLocalJacobian) Implementation;
    ...
    typedef typename GET_PROP(TypeTag, JacobianMatrix)::MatrixLittleBlockLowDim MatrixLittleBlock;
    typedef typename GET_PROP(TypeTag, JacobianMatrix)::MatrixLittleBlockLowDimCoupling CouplingMatrixLittleBlock;
    ...
    typedef Dune::MultiTypeBlockVector<CouplingMatrixBlock, MatrixBlock> LocalJacobian;
    ...
    typedef typename GET_PROP_TYPE(LowDimProblemTypeTag, Problem) Problem;
    typedef typename GET_PROP_TYPE(LowDimProblemTypeTag, Model) Model;
    typedef typename GET_PROP_TYPE(LowDimProblemTypeTag, GridView) GridView;
    typedef typename GET_PROP_TYPE(LowDimProblemTypeTag, FVElementGeometry) FVElementGeometry;
    typedef typename GET_PROP_TYPE(LowDimProblemTypeTag, ElementSolutionVector) ElementSolutionVector;
    typedef typename GET_PROP_TYPE(LowDimProblemTypeTag, PrimaryVariables) PrimaryVariables;
    typedef typename GET_PROP_TYPE(LowDimProblemTypeTag, VolumeVariables) VolumeVariables;
    typedef typename GET_PROP_TYPE(LowDimProblemTypeTag, ElementVolumeVariables) ElementVolumeVariables;
    typedef typename GET_PROP_TYPE(LowDimProblemTypeTag, ElementBoundaryTypes) ElementBoundaryTypes;
    typedef typename GET_PROP_TYPE(LowDimProblemTypeTag, LocalResidual) LocalResidual;
    ...
    LowDimLocalJacobian(const LowDimLocalJacobian &);
...
public:
    ...
    void assemble(const Element &element)
    {
        ...
        A_[lowDimIdx].setSize(numRows, numCols);
        A_[bulkIdx].setSize(numRows, numColsCoupling);
     ...
     *          primary variables
     */
    const MatrixLittleBlock &mat(typename SubProblemBlockIndices::LowDimIdx idx,
                                 const int i, const int j) const
    { return A_[idx][i][j]; }

    const CouplingMatrixLittleBlock &mat(typename SubProblemBlockIndices::BulkIdx idx,
                                         const int i, const int j) const
    { return A_[idx][i][j]; }
...
protected:
    ...
    {
        Valgrind::CheckDefined(globalProblemPtr_);
        return globalProblemPtr_->lowDimProblem();
    }
                                        ...
                                        const int pvIdx)
    {
        ...
        auto& priVars = globalProblem_().bulkProblem().model().curSol()[dofIdxGlobal];
                              ...
                              const ElementSolutionVector &partialDeriv)
    {
        ...
        {
            for (int eqIdx = 0; eqIdx < lowDimNumEq; eqIdx++)
            {
