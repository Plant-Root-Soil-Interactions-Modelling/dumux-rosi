...
#include <dumux/multidimension/model.hh>
...
public:
    MultiDimensionProblem(TimeManager &timeManager,
      ...
      newtonCtl_(asImp_())
    {
        ...
        useIterativeSolver_ = GET_PARAM_FROM_GROUP(TypeTag, bool, MultiDimension, UseIterativeSolver);
    ...
    Scalar nextTimeStepSize(const Scalar dt)
    {
        ...
        if ((startEpisodeWithTimeStepSize_ > 0) and timeManager().episodeIsFinished())
        {
          newDt = startEpisodeWithTimeStepSize_;
        }
    ...
    void episodeEnd()
    {
        ...
        if (startEpisodeWithTimeStepSize_ > 0)
        {
          ...
          bulkTimeManager().setTimeStepSize(startEpisodeWithTimeStepSize_);
          lowDimTimeManager().setTimeStepSize(startEpisodeWithTimeStepSize_);
	        //std::cout<<"set next time step "<<startEpisodeWithTimeStepSize_<<" "
          //                                << timeManager().timeStepSize()<<" "
          //                                << bulkTimeManager().timeStepSize()<<" "
          //                                << lowDimTimeManager().timeStepSize()<<"\n";
        }
        lowDimProblem().episodeEnd();
        bulkProblem().episodeEnd();
        //std::cout<<"set next time step "<<startEpisodeWithTimeStepSize_<<" "
        //                                  << timeManager().timeStepSize()<<" "
        //                                  << bulkTimeManager().timeStepSize()<<" "
        //                                  << lowDimTimeManager().timeStepSize()<<"\n";
    ...
    {
        DUNE_THROW(Dune::NotImplemented, "Restart multidimension simulations.");
    }
...
private:
    ...
    Scalar maxTimeStepSize_, initialTimeStepSize_;
