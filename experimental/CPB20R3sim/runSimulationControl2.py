...
def launchUQR(directoryN_,simInit,simStartSim, condition,spellDuration, simEnd):
    ...
    rprint = PhloemFluxPython(pl,psiXylInit = min(sx),ciInit = weatherInit["cs"]*0.5) 

    rprint = setKrKx_phloem(rprint)
    rprint = setFunctionalParameters(rprint, weatherInit)
    ...
    orgs_all = rprint.plant.getOrgans(-1, True)
    ...
    Q_in = 0
    Q_Exud_i = np.zeros(len(rprint.plant.nodes))
    Q_Exudmax_i = np.zeros(len(rprint.plant.nodes))
    Q_ST_init = np.array([])
    ...
    while simDuration < simMax: 
        ...
        """ Parameters phloem and photosynthesis """
        pl2 = pb.MappedPlant(seednum = 2) 
        pl2.readParameters("./input/" + name + ".xml")

        pl2.setGeometry(sdf) # creates soil space to stop roots from growing out of the soil
        pl2.initialize(verbose = True)
        pl2.simulate(simDuration, False)
        r = PhloemFluxPython(pl2,psiXylInit = min(sx),ciInit = weatherInit["cs"]*0.5) 
        ...
        #tiproots, tipstems, tipleaves = pl.get_organ_segments_tips()
        #Q_Exud_i[tiproots] = 0.2/12 * max(min(1 - sinusoidal(simDuration),1.),0.)
        #Q_Exud += Q_Exud_i
        ...
        if True:#(dtVTP >=0.29/24): #get results as VTP
            getVTPOutput(rprint, C_ST, np.array(r.psiXyl),[Q_Exud_i, Q_Exud,Q_Gr_i,Q_Grmax_i] ,vtpIndx, directoryN,  simStartSim, condition)
        ...
        rprint.plant.simulate(dt, False)
        
        """ for next time step """
        orgs = rprint.plant.getOrgans(-1, True)
        orgs_all = rprint.plant.getOrgans(-1, True)
        
        Ntbu = Nt
        Nt = len(rprint.plant.nodes)
...
if __name__ == '__main__':
    ...
    directoryN = "/" + scenario + sys.argv[3] + "Control2/"
