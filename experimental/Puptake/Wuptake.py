...
cell_number = [1,1,1]#[7, 7, 15]  # [8, 8, 15]  # [16, 16, 30]  # [32, 32, 60]  # [8, 8, 15] # [1]
periodic = False
...
dt =30 / (24 * 3600)  #  1/24/60#30 / (24 * 3600)  # time step [day]
...
IC_C = 0.1
s.setParameter("Soil.IC.C",str(IC_C))  # g / g water, = g/cm3 water 
...
if troubleShooting:
    ...
    print("rs.allDiff1d3dCW_rel",rs.allDiff1d3dCW_rel) # initial relative error. to compare with error during simulation
...
for i in range(0, NT):
    ...
    if rank == 0:
        ...
        proposed_inner_fluxes = np.array(r.segFluxes(rs_age + t, rx, rsx, approx = False, cells = False, soil_k = soil_k))*50  # [cm3/day]
    else:
    ...
    print('realized_mass_fluxes',np.max(np.abs(realized_mass_fluxes)))
    ...
    if troubleShooting:
        rs.checkMassOMoleBalance(sourceWat=soil_fluxes, # cm3/day 
                                  ...
                                  verbose_ = False,
                                  diff1d3dCW_rel_lim =np.full(10, np.Inf))
        print("rs.allDiff1d3dCW_rel",rs.allDiff1d3dCW_rel) #relative error between 1d and 3d
    ...
    if rs.allDiff1d3dCW_rel[0] > 1e-10:
        raise Exception
