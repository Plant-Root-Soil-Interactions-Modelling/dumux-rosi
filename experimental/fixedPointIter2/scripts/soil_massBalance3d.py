...
from rosi_richards import RichardsSP as RichardsNCSP  # C++ part (Dumux binding)
...
def solve(simtimes):
    ...
    s = setSoilParam(s)
    s.initialize()
    s.createGrid([-1e3, -1e3, -1.], [1e3, 1e3, 0.], [1,1,1], periodic = True)  # [cm]
    ...
    evap = -1 *0.
    s.setTopBC("constantFlux", evap) 
    s.setBotBC("freeDrainage")#"constantFlux", 0.) # "noFlux")#
    ...
    source_map = { 0: 0.03, 1:-0.01, 2: 0.02, 3:-0.04, 4: 0.03, 5:-0.06}
