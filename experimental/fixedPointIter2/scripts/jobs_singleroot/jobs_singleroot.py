...
def start_jobs(jobs):
    ...
    for job in jobs:
        ...
        soil_type, diffusion, sorption, decay = job
        job_name = soil_type + "_" + diffusion + "_"+ sorption + "_"+ decay
        print(job_name)
        ...
        with open(job_file, 'w') as fh:
            ...
            fh.writelines("mpiexec -n 1 python3 mainExudate_singleroot.py {:s} {:s} {:s} {:s}".format(soil_type, diffusion, sorption,decay))
...
def make_list():
    ...
    print("Creating", len(soil_type) * len(diffusion) *len(sorption)*len(decay), "simulations")
    print()
