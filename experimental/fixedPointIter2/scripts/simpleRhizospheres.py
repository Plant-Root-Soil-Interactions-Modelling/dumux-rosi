...
cell_number = np.array( [2,2,1]) #np.array( [1,1,1]) # 1cm3
...
def setBiochemParam(s):
    ...
    s.soil =  [0.03, 0.414, 0.038, 2, 1864]
    ...
    for i in range(s.numSoluteComp):
        s.setParameter( "Soil.IC.C"+str(i+1), str(0 ))# send data to dumux
    return s                          
...
def initialize_dumux_nc_(  gId=0, a_in=0.02,
    ...
    if a_in < a_out:
        ...
        # mol/cm2/day
        cyl.setParameter("Soil.IC.C1",cyl.dumux_str(c2[0]) ) 
        cyl.setParameter("Soil.IC.C2",cyl.dumux_str(c2[1]) ) 
        
        if len(Cells) > 0:#in case we update a cylinder, to allow dumux to do interpolation
        
            assert(len(c2[0])==len(Cells))
            ...
            j = 1
            cyl.setParameter("Soil.IC.C1Z",CellsStr) # m
            cyl.setParameter("Soil.IC.C2Z",CellsStr)
...
def getTBalance1d(dt,cyl,Wvolafter,Wvolbefore,Smassafter,Smassbefore):
    ...
    #print(rootSoilFluxesS,soilSoilFluxesS,Smassafter,Smassbefore)
    #print()
    #print()
...
def get1d3dBalance(dt):
    scvFluxes = s3d.getFluxesPerCell(0) * dt  # cm3
    scvFluxesS = s3d.getFluxesPerCell(1) * dt  # cm3
    #scvSourcesS = s3d.getSource(1) * s3d.getCellVolumes() * dt # kg
    #scvSourcesS2 = s3d.getSource(2) * s3d.getCellVolumes() * dt # kg
    ...
    print('\tRMSE 1d3d Wcell0:',scvFluxes[0]+(Wvolafter[0])- ( sum(cylsWVolAfter)))
    print('\tRMSE 1d3d Scell0:',scvFluxesS[0], Smassafter[0], cylsSAfter, sum(cylsSAfter))#soil_solute1d_perVoxelAfter,
...
def update_concentration( totContent, changeRatio, gradient,
    ...
    for gId, cyl in enumerate(cyls):
        ...
        for nComp in range(1, s3d.numComp):
            ...
            if (molKonzOld[nComp -1] != 0.).any():
                ...
                molKonzNew.append(
                    update_concentration(totContent = cOld*changeRatio+ max(konzLeftOver * deltaVol,0.),
                                                         changeRatio=changeRatio,gradient =gradientNew, 
                                                         volumes = volNew,isWater = False, verbose = False))
                if deltaVol < 0.:
                    konzLeftOver =  (cOld - sum(molKonzNew[nComp -1] * volNew))/(-deltaVol)

                
                if isDissolved:
                    molKonzNew[0] = getCSWfromC_total(cyl, molKonzNew[0], theta_new)# from C_S^l + C_S^s in mol/cm3 scv to C_S^l in mol/cm3 wat
...
for nn in range(2):
    ...
    cyls.append(initialize_dumux_nc_(x=initP,#[initP for i in range(NC-1)],
