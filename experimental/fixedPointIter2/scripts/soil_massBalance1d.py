...
def solve(simtimes, N, analytical = True):
    ...
    if analytical:
        ...
        s.setOuterBC("constantFluxCyl",0.1)  #  [cm/day] "noFlux")#
        s.setInnerBC("constantFluxCyl", -0.2) # "noFlux")#
    ...
    for r, dt in enumerate(dt_):
        ...
        print('getBoundaryFluxesPerFace_',s.getBoundaryFluxesPerFace_(0, length), s.molarMassWat )
        print('getFaceSurfaces_',s.getFaceSurfaces_(length))
        # cm/day
        print('self.base.getScvfBoundaryFluxes()[eqIdx]',s.base.getScvfBoundaryFluxes()[0][0] *  24 * 3600 / 1e4 * s.molarMassWat)
        ...
        if rank == 0:
            print('\tChange in water volume [cm3] per voxel:',sum(Wvolafter-Wvolbefore), rootSoilFluxes)
