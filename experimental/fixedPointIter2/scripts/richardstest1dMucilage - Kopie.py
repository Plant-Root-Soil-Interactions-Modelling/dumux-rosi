...
def initialize_dumux_nc_(  gId=0, a_in=0.02,
    ...
    if a_in < a_out:
        ...
        cyl.maxDt = 10/(3600*24) # soilModel.maxDt_1DS
...
def doPrints(cyl,a_in,timeT, initMolm):
    print('CC2',(cyl.getCellCenters_().reshape(-1)[[0,-1]]-a_in)/100,'m')
...
#points = np.logspace(np.log(0.02) / np.log(lb), np.log(0.03) / np.log(lb), 
#                              NC, base = lb)
...
cc = (cyl.getCellCenters_().reshape(-1)-a_in)/100.
getPressureHead = [[],[],[]]
Cmucil = [[],[],[]]
theta = [[],[],[]]
Viscosity = [[],[],[]]
...
with open("results/results_v9/dataDumux.pkl", "wb") as file:
