...
def doPrints(cyl,a_in,timeT, initMolm):
    ...
    #print('mucil',cyl.getSolution(2)[[0,-1]]*rhoWM*1e3)# mmol/cm3
...
cyl = initialize_dumux_nc_(x=[-100. for i in range(NC-1)],a_in=a_in,a_out=a_out,
                            c2 = [mlFr * CC[i] * 3e3 for i in range(NC-1)], 
...
Cmucil[0] = np.zeros(len(getPressureHead[0]))#cyl.getSolution(2)*rhoWM
...
initCls = 0#sum(cyl.getSolution(2)*rhoWM*cyl.getWaterContent()*cyl.getCellSurfacesCyl().reshape(-1))*100 # 

for i in range(2):
    print(cyl.numSoluteComp)
    #cyl.ddt = 1e-3
    cyl.solve(10.)#36000/24/3600/2, saveInnerDumuxValues_ =True)
    timeT += 36000./2.
    ...
    print('end')
    Cmucil[i+1] = np.zeros(len(getPressureHead[0]))#cyl.getSolution(2)*rhoWM 
    theta[i+1] = cyl.getWaterContent()
    Viscosity[i+1] = np.array(cyl.base.getViscosity())
    getPressureHead[i+1] = np.array(cyl.base.getPressureHead())*100.
...
for i in range(3):
    axes[0, 1].plot(cc, Cmucil[i]*1e3, label=str(18000 * i))
axes[0, 1].grid(True)
axes[0, 1].legend()
...
with open("results/dataDumux.pkl", "wb") as file:
