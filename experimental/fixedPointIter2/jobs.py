...
def start_jobs(jobs):
    ...
    for job in jobs:
        ...
        soil_type, res, simMax, exudate, decay = job
        job_name = soil_type + "_" + res + "_"+ simMax +"_" + exudate+"_"+decay
        print(job_name)
        ...
        with open(job_file, 'w') as fh:
            ...
            fh.writelines("mpiexec -n 1 python3 mainExudate.py {:s} {:s} {:s} {:s}".format(soil_type, res, simMax, exudate, decay))
...
def make_list():
    ...
    print("Creating", len(soil_type) * len(res) * len(simMax)* len(exudate)*len(decay), "simulations")
    print()
