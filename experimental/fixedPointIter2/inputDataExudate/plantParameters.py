...
def prescribed_exudation(soil_type, ifexu):
    ...
    if ifexu: 
        if soil_type == 'loam':
            exu_prop = np.array([1.11E-05, 1.11E-05,5.27E-06,5.27E-06])#[mol C/(cm2 root surface  day)] 
        elif soil_type == 'sand':
            exu_prop = np.array([1.79E-05, 1.79E-05,4.14E-06,4.14E-06]) #[mol C/(cm2 root surface day)]
        else:
            print('No exudate properties found')
    else: 
        exu_prop = np.array([0.,0.,0.,0.,0.]) #[mol/(cm2 day)]
...
def SorptionParams(soil_type, sorp): 
    kads = [[5100,581,185],[867,98.77, 31.45]] #cm^3 water/ mol/ d
