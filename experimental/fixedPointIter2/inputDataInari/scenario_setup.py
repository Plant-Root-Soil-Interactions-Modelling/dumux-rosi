...
def create_soil_model( usemoles, results_dir ,
                        p_mean_ = -100,paramIndx =0,
                     noAds = False, ICcc = None, doSoluteFlow = True,
                     MaxRelativeShift = 1e-8,doSoluteUptake = False,
...
def setupOther(s, p_mean_):
    ...
    pressureinit = s.getSolutionHead_()    
    thetainit = s.getWaterContent_()
    
    assert('dumux and theoretical water retention curves are different. set lower eps_regularization' and (abs(
    (vg.pressure_head(thetainit,s.vg_soil)-pressureinit)/pressureinit) < 1e-3).all()) 
...
def create_mapped_plant(initSim, soil_model, fname, path, 
    ...
    perirhizalModel.RS_Uptake_Vmax = 0#6.56 *  1e-12 * 24 * 3600 # pmol / cm² / s =>  mol / cm² / d
    perirhizalModel.RS_Uptake_km =  0#7.1 *1e-3*1e-6# mumol/L=> mol/cm3
    # to do: P sorbtion:
