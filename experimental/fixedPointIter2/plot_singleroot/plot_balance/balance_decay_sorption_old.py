...
scenario = 'day1_loam_1_exu_withdecay_withsorptionlow'
...
plt.plot(x_sim-dt+starttime, df['Exud_ads'].loc[:].values[:len(x_sim)]+df['Exud_liq'].loc[:].values[:len(x_sim)]+df['Exud_decay'].loc[:].values[:len(x_sim)],  linestyle = '--', label = 'Exud_tot_test')
