    ...
    for j in range(0,sim_end):
        ...
        for i in range(0, len(polylines)):
            ...
            for k in range(0, len(polylines[i])-1):
                ...
                if polylengths[i]>=lmax[roottype]*0.99:
                    #print('REACHED')
                    kexu = kex_/2
                    c = 1
                #if artificial shoot 
...
#Root system direct 
times = [0, 20]
exu_prop = np.array([0.001,0.001])#[kg C/(m2 root surface  day)] 
...
for i in range(0, len(scenarios)): 
    scenario = 'test_'+scenarios[i]+'_res1_sorptionlow_SWP_ini100_trans0'
    ...
    if i == 0: 
        ...
        plt.plot(x_sim-dt+starttime, df['Exud_ads'].loc[:].values[:len(x_sim)],  color = cols[0], linestyle = linst[i])
        plt.plot(x_sim-dt+starttime,  df['Exud_liq'].loc[:].values[:len(x_sim)], color = cols[1], linestyle = linst[i])
        plt.plot(x_sim-dt+starttime, df['Exud_decay'].loc[:].values[:len(x_sim)], color = cols[2], linestyle = linst[i])
   
for i in range(0, len(scenarios)): 
    plt.plot(np.nan, np.nan, color = 'k', linestyle = linst[i], label = scenarios[i])
plt.legend()
