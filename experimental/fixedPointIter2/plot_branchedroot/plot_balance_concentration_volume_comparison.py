...
annots = ['(a)','(b)','(c)']
figs = ['branched_root1', 'straight_root1']
linst = ['-', '--']
dt = 20/60/24 #day
cols= ['r', 'b', 'g', 'y']
...
for m, axb in enumerate(ax_bottom):
    ...
    for i in range(0, np.shape(conc)[0]): 
        ...
        if time[i] == time[-1]:
            x = conc[i,:]
            y = watvol_cum[i,:]
            line = ax_bottom[0].plot(x,y, color = 'k', linestyle = linst[m], label = titles[m])
            ax_bottom[0].ticklabel_format(style='sci', axis='x', scilimits=(0, 0))
    ...
    ab = AnnotationBbox(im, (1, 0), xycoords='axes fraction', box_alignment=(1.5+m*0.75,0.2), frameon = False)
    ax_bottom[1].add_artist(ab)
    ax_bottom[1].axis('off')
