...
for m, axb in enumerate(ax_bottom):
    ...
    count = len(list(folder.glob("*time*")))
    print(path2file+"/"+scenario)
    print(count) 
    sys.exit()
    for i in range(0, count): 
        ...
        if i == 0: 
            ...
            cellvol_tot = np.hstack([cellvol_tot, cellvol_tot_])
    ...
    for i in range(0, np.shape(conc)[0]): 
        ...
        # if np.around(int(time[i] *1000)/1000-int(time[i]),2) == 0.5 : #only plot lunch time concentration/volumes for better visibility
            x = conc[i,:]
            y = watvol_cum[i,:]
            line = ax_bottom[0].plot(x,y, color = colors[i])
        # ax[1].set_title(add_labels[i][j])   
        

    # axb.set_xscale('log')
    # axb.set_yscale('log')
    axb.set_xlim([1e-8,1e-4])
    axb.set_ylim([0, 0.1])
