...
exud_out = df['Exud_tot'].loc[:].values+(-1*df['Exud_decay'].loc[:].values)  #mol
print('exu_tot')
...
plt.plot(x_sim-dt, exud_in, '.', label = 'prescribed incoming\n cumulative plant exudation') 
plt.plot(x_sim-dt, exud_out[:len(x_sim)],label = 'Exud_tot + Exud_decay')
plt.plot(x_sim-dt, df['Exud_ads'].loc[:].values[:len(x_sim)],  label = 'Exud_sorbed')
plt.plot(x_sim-dt,  df['Exud_liq'].loc[:].values[:len(x_sim)],  label = 'Exud_liq')
plt.plot(x_sim-dt, -1*df['Exud_decay'].loc[:].values[:len(x_sim)],  label = 'Exud_decay')
plt.plot(x_direct,y_direct, label = 'direct calculation')
plt.legend()
plt.title('decay & sorption')
plt.xlabel('Time (d)')
