...
class fixedPointIterationHelper():
    ...
    def __init__(self, s, perirhizalModel, plantModel, 
        ...
        self.seg_sol_fluxes = np.array([Q_Exud_i/dt, Q_mucil_i/dt])# mol/segment/day   
    ...
    def initializeOldData(self, seg_fluxes, outer_R_bc_wat, outer_R_bc_sol):
        ...
        self.outer_R_bc_sol_old =  np.zeros(self.s.numDissolvedSoluteComp)  # inter-cell solute flux in 3d soil model
    ...
    # each solute for each cyl.
        ...
        if rank == 0:
            ...
            proposed_outer_sol_fluxes = np.full((self.s.numDissolvedSoluteComp, self.numSegs), 0.)
    ...
    def compute1dChangesSolute(self, dt):
        ...
        if rank == 0:
            ...
            # store error
            self.s.errSoil_source_sol_abs = sum(sources_sol_from1d.flatten())*dt - (sum(self.Q_Exud_i) + sum(self.Q_mucil_i))

            if perirhizalModel.debugMode:
                print("self.s.errSoil_source_sol_abs",self.s.errSoil_source_sol_abs,"(sum(self.Q_Exud_i) + sum(self.Q_mucil_i))",
                      (sum(self.Q_Exud_i) + sum(self.Q_mucil_i))," sum(sources_sol_from1d.flatten())", sum(sources_sol_from1d.flatten())*dt)
