...
def plot_plant(plant, p_name, vals=[], render = True, interactiveImage = True, filename = "plot_plant", range_ = None):
    ...
    datas_p = []
    for ddindx, dd in enumerate(vals):
        dd_p = np.array(dd)
        dd_p[abs(dd_p) < cutoff] = 0
        datas_p.append(dd_p)
        ana.addData(p_name[ddindx], dd_p)
            
    pd = segs_to_polydata(ana, 1.,p_name_,  vals)  # poly data
    tube_plot_actor, color_bar = plot_roots(pd, p_name_[-1], "", render = False)
...
# TODO: finish
def plot_plants_and_soil(plantModel,soilModel, 
                        periodic:bool, min_b, max_b, 
    ...
    """
    if rank == 0:
        plot_plant(plant=plantModel, p_name=pnamePlants, 
                vals=pPlants, render = False, interactiveImage = False, filename = filename, range_ = [0,5000])
