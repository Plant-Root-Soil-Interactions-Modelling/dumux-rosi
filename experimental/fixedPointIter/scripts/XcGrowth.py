...
def XcGrowth(initsim, mode,simMax,extraName,paramIndx_,spellData):
    ...
    s, soil = scenario_setup.create_soil_model(demoType = mode, times = None, 
    ...
    rs, r = scenario_setup.create_mapped_plant( mode,initsim, s, xml_name,
    ...
    while rs_age < simMax:
        ...
        for nc in range(rs.numSoluteComp):# normally all 0 for nc >= numDissolvedSoluteComp
        ...
        if False:
            try:
                ...
                print('issue Q_Exud_inflate',rank, 
                      ...
                      Q_Mucil_inflate, sum(Q_Mucil),abs(Q_Mucil_inflate- sum(Q_Mucil)) )
                raise Exception
        ...
        while cL or failedLoop:
            ...
            net_sol_flux_, net_flux_, seg_fluxes__, real_dtinner,failedLoop, n_iter_inner_max = cyl3.simulate_const(s, 
                                                    ...
                                                    results_dir = results_dir,
                                                        k_iter_ = k_iter,outer_n_iter = n_iter,
        ...
        if True:
            ...
            for i in range(rs.numDissolvedSoluteComp):
                #comm.barrier()
                if mpiVerbose and rank==0:# or (max_rank == 1):
                ...
                write_file_array("Soil_solute_conc"+str(i+1), 
                                 ...
                                 directory_ =results_dir, fileType = '.csv') 
            for i in range(rs.numDissolvedSoluteComp, rs.numSoluteComp):
                #comm.barrier()
                if mpiVerbose and rank==0:# or (max_rank == 1):
                    print(rank,"Soil_solute_conc"+str(i+1))
                #comm.barrier()
        ...
        if (mode != "dumux_w"):
            ...
            write_file_float("Q_Exud", Q_Exud, directory_ =results_dir)
            write_file_float("Q_Mucil", Q_Mucil, directory_ =results_dir)
        ...
        for lId, cyl in enumerate(rs.cyls):
            if not isinstance(cyl, AirSegment):
                ...
                write_file_array("Cyl_cellVol_"+str(gId),
                                 ...
                                 directory_ =results_dir+'cyl_val/', allranks = True)
                for ccc in range(rs.numSoluteComp):
                    sol0 = np.array(cyl.getContent(ccc +1, ccc < rs.numDissolvedSoluteComp)).flatten()
        ...
        for konz in np.array([True]):#, False]):
            ...
            for i in range(1, rs.numComp):
                ...
                if not konz:
                    ...
                    if i <= rs.numDissolvedSoluteComp:
