...
def simulate_const(s, rs, sim_time, dt, rs_age, Q_plant,
    ...
    global_isRootSeg = r.getXcyl(data2share=local_isRootSeg,idCyll_=None, doSum = False, reOrder = True)
    ...
    if(len(outer_R_bc_sol[0]) == 0):
        outer_R_bc_sol = np.full((r.numSoluteComp,s.numberOfCellsTot), 0.)  
    ...
    for Ni in range(N):
        ...
        if True:
            write_file_array('change_soil',np.array([rs_age_i_dt, (r.spellData['scenario'] != 'none'),
                                               ...
                                               (rs_age_i_dt > r.spellData['spellStart']) , 
                                                ...
                                                r.spellData['spellStart'],r.spellData['spellEnd'],
                                                     r.spellData['condition']   ]), 
        ...
        if (r.spellData['scenario'] != 'none') and (r.spellData['scenario'] != 'baseline'):
            if  ((rs_age_i_dt > r.spellData['spellStart']) and (not r.enteredSpell)) or ((rs_age_i_dt > r.spellData['spellEnd']) and (not r.leftSpell)):#
                ...
                if rank ==0:
                    print('error before change',r.sumDiff1d3dCW_rel ,r.sumDiff1d3dCW_abs,'pheadinit_cm',pheadinit_cm, doChange)
                if doChange:# but then i also need to adaptthe water mol fraction of the solutes to keep the same content
                    nc_content = np.array([comm.bcast(s.getContent(nc+1, nc < 2), root=0)  for nc in range(r.numDissolvedSoluteComp)])# mol
                    if not doMinimumPrint:
                    ...
                    for nc in range(r.numDissolvedSoluteComp):
                    ...
                    for locIdCyl, cyl in enumerate(r.cyls):
                        if not isinstance(cyl, AirSegment):
                            ...
                            nc_content = np.array([cyl.getContentCyl(nc+1, nc < r.numDissolvedSoluteComp)  for nc in range(r.numDissolvedSoluteComp)])# mol
                            ...
                            nc_content_new = np.array([cyl.getContentCyl(nc+1, nc < r.numDissolvedSoluteComp)  for nc in range(r.numDissolvedSoluteComp)])# mol
                    ...
                    for lId, cyl in enumerate(r.cyls):
                        if not isinstance(cyl, AirSegment):
                            ...
                            write_file_array("aftChCyl_cellVol_"+str(gId),
                                             ...
                                             directory_ =results_dir+'cyl_val/', allranks = True)
                            for ccc in range(r.numSoluteComp):
        ...
        if( ((np.floor(max(r.sumDiff1d3dCW_rel - r.sumDiff1d3dCW_relOld)) > 1.) \
            ...
            if (r.sumDiff1d3dCW_abs[issueComp] > 1e-13).any():
                ...
                for nc in range(r.numSoluteComp):# normally all 0 for nc >= numDissolvedSoluteComp
                        write_file_array("error_sol_content_diff1d3dabs"+str(nc+1), 
                                         r.allDiff1d3dCW_abs[nc+1], directory_ =results_dir, fileType = '.csv')
        ...
        while  continueLoop(r,n_iter, dt,False,float(Ni) * dt,'inner_loopdata', isInner = True, plant = rs):
            ...
            if rank == 0:
                ...
                soilKOut = s.vg_soil.Ksat  /dist_factor# # [cm/d]  / [cm]  = day-1
                soilK = soilKIn
                
                if( len(seg_fluxes) > 0.) and not (r.beforeAtNight and (r.weatherX["Qlight"] == 0.)):
            ...
            rhizoTotCBefore_eachC = np.array([r.getContentCyl(idComp=idC) for idC in range(1,r.numComp)])
            ...
            if not doMinimumPrint:
                for nc in range(r.numSoluteComp):
            ...
            soil_solute = np.array( [np.array(r.getC_rhizo(s.numberOfCellsTot, idComp = idc + 1, konz = False)) for idc in range(r.numSoluteComp)])
            if r.mpiVerbose:# or (max_rank == 1):
                if rank == 0:
            ...
            rhizoTotCAfter_eachC = np.array([r.getContentCyl(idComp=idC) for idC in range(1,r.numComp)])
            ...
            if r.mpiVerbose:# or (max_rank == 1):
                ...
                #comm.barrier()
            seg_fluxes_limited = r.getXcyl(data2share=r.seg_fluxes_limited,idCyll_=None, doSum = False, reOrder = True) 
            seg_fluxes_limited_Out = r.getXcyl(data2share=r.seg_fluxes_limited_Out,idCyll_=None, doSum = False, reOrder = True) 
            seg_fluxes_limited_sol_Out = r.getXcyl(data2share=r.seg_fluxes_limited_sol_Out,idCyll_=None, doSum = False, reOrder = True) 
            seg_fluxes_limited_mucil_Out = r.getXcyl(data2share=r.seg_fluxes_limited_mucil_Out,idCyll_=None, doSum = False, reOrder = True) 
            seg_fluxes_limited_sol_In = r.getXcyl(data2share=r.seg_fluxes_limited_sol_In,idCyll_=None, doSum = False, reOrder = True)
            seg_fluxes_limited_mucil_In = r.getXcyl(data2share=r.seg_fluxes_limited_mucil_In,idCyll_=None, doSum = False, reOrder = True) 
            ...
            r.new_soil_solute = np.array( [np.array(r.getC_rhizo(s.numberOfCellsTot, idComp = idc + 1, konz = False)) for idc in range(r.numSoluteComp)])
            try:
            ...
            assert soil_source_sol.shape == (r.numSoluteComp, s.numberOfCellsTot)
            ...
            soil_solute_content_beforeReset = comm.bcast(np.array([np.array(
                s.getContent(i+1, isDissolved = (i < r.numDissolvedSoluteComp))) for i in range(r.numSoluteComp)]),
                                                root = 0) # mol
            ...
            assert soil_sources_limited.shape == (s.numComp, s.numberOfCellsTot)
            assert soil_contents.shape == (s.numComp, s.numberOfCellsTot)
            ...
            while redoSolve:
                ...
                try:
                    ...
                    s.reset()
                    for ncomp in range(r.numSoluteComp):
                        try:
            ...
            outer_R_bc_wat = outer_R_bc[0]# [cm3] 
            ...
            soil_solute_content_new = comm.bcast(np.array([np.array(s.getContent(i+1, isDissolved = (i < r.numDissolvedSoluteComp))) for i in range(r.numSoluteComp)]), root = 0) # mol
            if r.mpiVerbose:# or (max_rank == 1):
                #comm.barrier()
                if rank == 0:
            ...
            assert outer_R_bc_sol.shape == (r.numSoluteComp, s.numberOfCellsTot)
            ...
            for nc in range(r.numDissolvedSoluteComp, r.numSoluteComp):
                ...
                # thus, for elements which do not flow (range(r.numDissolvedSoluteComp, r.numSoluteComp)), there are no changes
            ...
            if  (not doMinimumPrint):#(n_iter % skip == 0) and 
                    ...
                    write_file_array("fpit_sources_wat", 
                                     sources_wat, 
                                     directory_ =results_dir, fileType = '.csv') 
