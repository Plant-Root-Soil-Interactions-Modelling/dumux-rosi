...
#include "./richards2c/richards2cbuffermodel.hh"
...
//    void postTimeStep()
//    {
//        std::cout << "SET TIME STEP TO: " << adaptedDt_ << std::endl;
//        this->timeManager().setTimeStepSize(adaptedDt_);
//    }
    /*!
...
//        Scalar dx_=GET_RUNTIME_PARAM(TypeTag, Scalar, SpatialParams.rootRadius);
//        adaptedDt_=1/
//                    ((elemVolVars[scvIdx].effDiffCoeff()/(dx_*dx_))
//                    +
//                    sourceValues[conti0EqIdx]/element.geometry().volume()/
...
//        std::cout << "ADAPTED TIME STEP: " << adaptedDt_ <<std::endl;
    //
	// print out flux.Var
    //    	    int fIdxInner = 0;
    ...
    void addOutputVtkFields()
    {
        ...
        ScalarField& L1NormC = *(this->resultWriter().allocateManagedBuffer(numDofs));
        ScalarField& relErrS = *(this->resultWriter().allocateManagedBuffer(numDofs));
        ScalarField& L1NormS = *(this->resultWriter().allocateManagedBuffer(numDofs));
        ...
        //ScalarField& Timestepsize = *(this->resultWriter().allocateManagedBuffer(numDofs));
        ...
        for (const auto& element : elements(this->gridView()))
        {
            ...
            //            }
                    ...
                    L1NormC[dofGlobalIdx]=std::abs((elemVolVars[scvIdx].massFraction(transportCompIdx)-AnalyticalC[dofGlobalIdx]));
                    ...
                    L1NormS[dofGlobalIdx]=std::abs((sourceC[dofGlobalIdx]-AnalyticalS[dofGlobalIdx]));
                    //std::cout <<" TIME !!!"<< this->timeManager().time() <<std::endl;
                    //Timestepsize[dofGlobalIdx]=1/
                    //        ((elemVolVars[scvIdx].effDiffCoeff()/(rootRadius_*rootRadius_))
                    //        +
                    //        AnalyticalS[dofGlobalIdx]/element.geometry().volume()/
                    //        (elemVolVars[scvIdx].density() * elemVolVars[scvIdx].massFraction(transportCompIdx) *
                    //        (elemVolVars[scvIdx].saturation(phaseIdx)*elemVolVars[scvIdx].porosity()+elemVolVars[scvIdx].buffer())));
                }
            }
    ...
    //    std::cout<< " " << std::endl;
        ...
        this->resultWriter().attachDofData(L1NormC, "L1NormC", isBox);
        this->resultWriter().attachDofData(L1NormS, "L1NormS", isBox);
        //this->resultWriter().attachDofData(Timestepsize, "Timestepsize", isBox);
