...
//#endif
                                  ...
                                  const ElementVolumeVariables &elemVolVars) const
    {
        ...
        else
        {
            ...
            Scalar dtRhizosphere = this->timeManager().timeStepSize()/GET_RUNTIME_PARAM(TypeTag, Scalar, radialTimeManager.TimeStepRatio);
            ...
            Scalar rhizosphereVolume = rootIntersectionLength*(M_PI*rootRadius*rootRadius)/this->couplingManager().lowDimVolume(element)
            ...
            OnedRadialSol.setInitialSoilNutrient(GET_RUNTIME_PARAM(TypeTag, Scalar, BoundaryConditions.InitialSoilFracK));

            //set boundary conditions
            PrimaryVariables coupledFluxes = this->evalFluxes(element, fvGeometry, scvIdx, elemVolVars);
            Scalar surfaceWeight = (2*M_PI*rootRadius*rootIntersectionLength)/this->couplingManager().lowDimSurface(element);
            coupledFluxes *= surfaceWeight/rootIntersectionLength;
            ...
            PrimaryVariables TransportValue = OnedRadialSol.ActiveUptakeNumerical();
            TransportValue *= -2*M_PI*rootRadius;
    ...
    PrimaryVariables evalFluxes(const Element &element,
                                  const FVElementGeometry &fvGeometry,
                                  const int scvIdx,
                                  const ElementVolumeVariables &elemVolVars) const
    {
        FluxVariables fluxVars;
        PrimaryVariables fluxes_;
        fluxes_ = 0;
        ...
        for (int scvfIdx = 0; scvfIdx < fvGeometry.numScvf; scvfIdx++)
        {
            ...
            auto faceDiffFlux = (fluxVars.massFractionGrad(transportCompIdx)*fluxVars.face().normal)
                                                *fluxVars.porousDiffCoeff()*elemVolVars[scvIdx].density();
            auto faceAdvFlux = -fluxVars.volumeFlux(phaseIdx) * elemVolVars[scvIdx].density()*
                                                elemVolVars[scvIdx].massFraction(transportCompIdx);
            fluxes_[transportEqIdx] += faceDiffFlux + faceAdvFlux;
            fluxes_[conti0EqIdx] += fluxVars.volumeFlux(phaseIdx)*elemVolVars[scvIdx].density();
            diffFluxTotal += faceDiffFlux;
            avdFluxTotal += faceAdvFlux;
        }
        std::cout<<"Pelect No: "<< abs(avdFluxTotal/diffFluxTotal)<<"\n";
        //std::cout<<" fluxes_[conti0EqIdx] "<< fluxes_[conti0EqIdx];
        //std::cout<<" fluxes_[transportEqIdx] "<< fluxes_[transportEqIdx];
        return fluxes_;
    }
    // \}
    /*!
