...
public:
    ...
    : ParentType(timeManager, gridView)
    {
        ...
        pressureRoot_ = tree.template get<Scalar>("BoundaryConditions.InitialRootPressure");
        compFracRoot_ = tree.template get<Scalar>("BoundaryConditions.InitialRootFracK");
...
//#endif
                      ...
                      const ElementVolumeVariables &elemVolVars) const
    {
        ...
        auto posInnerRadius = std::max(element.geometry().corner(0)[0],element.geometry().corner(1)[0]);
        if (posOuterRadius > this->bBoxMax()[0] - eps_)
        {
            values[conti0EqIdx] = tree.template get<std::vector<Scalar>>("BoundaryConditions.OuterFluxes")[conti0EqIdx];
            values[transportEqIdx] = tree.template get<std::vector<Scalar>>("BoundaryConditions.OuterFluxes")[transportEqIdx];
...
//    {
//        return this->timeManager().willBeFinished() ? true : false;
//    }
    /*!
...
private:
    ...
    Scalar pnRef_, Vmax_, Km_, rootRadius_, diffustionRootMembrane_, compFracRoot_, sigma_, pressureRoot_ ;
